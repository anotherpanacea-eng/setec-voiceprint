        ...
        *(
            render_freq_table_phase_block(
                ...
                (
                    ...
                    f"same top-300 corpus-aggregate selection, full-"
                    f"family relative frequencies preserved (no "
                    f"selected-subset renormalization, matching "
                    f"production `stylometry_core.py`). Cell-by-cell "
                    f"agreement here verifies that SETEC's n-gramming + "
                    f"frequency-table-construction code path matches a "
                    f"from-scratch reimplementation on production-"
                    f"shaped vectors, independent of the distance math. "
                    f"setec-only and stylo-only feature counts should "
                    f"be 0 (both sides select the same top-K from the "
                    f"same corpus). Row sums are typically < 1.0 -- the "
                    f"mass not captured by the top-K is the share of "
                    f"features outside the selection."
                ),

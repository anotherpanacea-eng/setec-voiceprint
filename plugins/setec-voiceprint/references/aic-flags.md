...
*Baselines drift too.* A marker word elevated against an older or cross-venue baseline is not evidence of AI prose on its own. Kosch, Welsch, Hedderich and Katins (arXiv 2609.23090, CHI 2027) read 14,262 CHI papers from 1982 to 2026. They found a word-level shift toward density whose onset, in 2021 to 2022, precedes the first cohort of authors who could have used LLMs. It coincides with CHI's switch from page to word limits, though the authors report the shift is the same across paper lengths. Their conclusion is that LLMs accelerated the drift rather than starting it. Published marker lists also went stale fast: *delve* ran 0.07, 0.64 and 0.06 per 10,000 tokens in 2022, 2024 and 2026, and only 6 of 17 listed markers still showed a doubling by 2026. Function words separated eras about as well as content words (AUC 0.864 vs 0.857), so era drift is stylistic as well as topical; the authors also name topic change as a possible explanation. It is one venue, and marker rankings may not transfer across venues. Three rules follow:

- State the baseline's date range, from the manifest's `date_written` or, failing that, its `era` bucket. Treat a lexical elevation against an undated baseline (including the provisional entries in the repository-root `baselines/register_typical.yaml`) as screening only, never as evidence. For recent targets the closest-period baseline is `post_ai_widespread`, which carries its own contamination risk, so report against both a `pre_chatgpt` and a period-matched baseline.
- Prefer the convergence habit (one prestige term reused across unrelated contexts) over list hits. The habit survives word-list turnover; the list does not.
- Treat period register as its own baseline. Historical or period-pastiche prose carries its era's vocabulary, and its marker profile is evidence about era, not authorship (see the domain-shift guardrail in `distributional-diagnostics.md`).

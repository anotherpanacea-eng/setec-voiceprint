...
*Baselines drift in human prose too.* A marker word elevated against an older or cross-venue baseline is not evidence of AI prose on its own. Kosch, Welsch, Hedderich and Katins (arXiv 2609.23090, CHI 2027) read 14,262 CHI papers from 1982 to 2026 and found a word-level shift toward density that began in 2021 to 2022, at least a conference cycle before authors could have used LLMs, and coincided with a venue format change. Several published AI markers spiked and then retreated in human-authored papers: *delve* went from 0.07 to 0.64 to 0.06 per 10,000 tokens across 2022, 2024 and 2026, and only 6 of 17 listed markers still showed a doubling by 2026. Content words separated eras almost as well as function words (AUC 0.857 vs 0.864), so topic drift alone can look like a style signature. Three rules follow:

- Compare lexical frequencies only against a baseline from the same period and venue or register, and state the baseline's date range. An undated baseline supports no lexical claim.
...
- Treat period register as its own baseline. Historical or period-pastiche prose carries its own era vocabulary, and its marker profile is evidence about era, not authorship.

...
| `author_corpus_export` | experimental | No | --source-manifest (required); --register-map (required); --allowed-ai-status (required); --persona (required); --hmac-key (required); --output-dir (required) | stdlib | CPU / stdlib (+ optional spaCy model) | — |
...
| `passage_remediation` | Heuristic (uncalibrated) | No | owner-private root containing every bound member. (required); private descriptor for a spec-36 passage inventory and projection receipt. (required); One closed private descriptor resolving an exact-hash-bound, manifest-derived spec-36 passage inventory and projection receipt under the same retained private root. (required) | stdlib | CPU / stdlib (+ optional spaCy model) | — |
...
| `shingle_dedup` | Heuristic (uncalibrated) | Yes | explicit staged-descriptor JSONL or exact-pinned local index, as required by the selected mode (required); strict UTF-8 staged-descriptor JSONL plus local bounded UTF-8 documents; build-index writes a named index and checkpoint directory, query-doc reads an exact-pinned index plus a named query document and writes a report, and batch-report reads an exact-pinned index and writes a report/checkpoint. (required); for a compatible immutable checkpoint directory (optional) | stdlib | CPU / stdlib (+ optional spaCy model) | 8 |

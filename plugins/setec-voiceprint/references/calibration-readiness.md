...
| `near_dup_dedup` | stable | No | a `corpus_manifest.jsonl` to validate (required); FILE to choose the exact-bound passage-mode checkpoint. (optional); to restore completed passage/document shards and stages from that checkpoint (or the automatic opaque temp checkpoint). (optional) | req: datasketch | CPU + network | — |

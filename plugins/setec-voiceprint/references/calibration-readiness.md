...
| `agd_move_scan` | Heuristic (uncalibrated) | Yes | --judge (required); pre-computed judge feature manifest (optional); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | 120 |
...
| `argquality_dimension_profile` | Literature-anchored | Yes | --judge (required); pre-computed judge feature manifest (optional); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | 120 |
...
| `enthymeme_gapflag` | Literature-anchored | Yes | --target (required) | stdlib | CPU / stdlib (+ optional spaCy model) | 120 |
| `fallacy_scan` | Literature-anchored | Yes | --judge (required); pre-computed judge feature manifest (optional); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | 120 |
...
| `general_imposters` | Literature-anchored | No | labeled corpus (required); --target (required); --candidate-persona (required) | req: scipy | CPU / stdlib (+ optional spaCy model) | — |
...
| `mimicry_cosplay_audit` | Heuristic (uncalibrated) | Yes | --target (required) | stdlib | CPU / stdlib (+ optional spaCy model) | — |
...
| `narrative_decision_long_form` | experimental | Yes | pre-computed judge feature manifest (optional); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | 25001 |
...
| `pov_voice_profile` | Heuristic (uncalibrated) | No | labeled corpus (required) | stdlib; opt: spacy | CPU / stdlib (+ optional spaCy model) | 3000 |
...
| `s5_distance` | Empirical (provisional) | Yes | nothing required to run; add a baseline / labeled corpus to calibrate | stdlib | CPU / stdlib (+ optional spaCy model) | — |
...
| `verbatim_mosaic_audit` | Heuristic (uncalibrated) | Yes | nothing required to run; add a baseline / labeled corpus to calibrate | stdlib | CPU / stdlib (+ optional spaCy model) | 24 |
...
| `voice_verifier` | Literature-anchored | No | labeled corpus (required); --query (required); --reference (required); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | — |
| `warrant_probe` | Literature-anchored | Yes | --judge (required); pre-computed judge feature manifest (optional); LLM API access (key + per-call cost) (required) | stdlib | No local GPU; LLM API access (network + key + per-call cost) | 120 |
...
| `acquire_pdf_urls` | Operational tooling (calibration N/A) | Yes | a local URL-list file (JSONL objects with a url key, or bare PDF URLs) (required) | req: pypdf, requests | CPU + network | 1500 |
| `acquire_stackexchange` | Operational tooling (calibration N/A) | Yes | local Stack Exchange site data dump containing Posts.xml and Users.xml (required); py7zr is needed only when --dump is a .7z rather than an extracted directory (optional) | stdlib; opt: py7zr | CPU + local I/O | 0 |
| `author_corpus_export` | experimental | No | --source-manifest (required); --register-map (required); --allowed-ai-status (required); --persona (required); --hmac-key (required) | stdlib | CPU / stdlib (+ optional spaCy model) | — |
| `build_opengrants_zenodo_source_list` | Operational tooling (calibration N/A) | Yes | a local weecology/ogrants Git worktree, full commit SHA, and cached or live Zenodo record metadata (required) | req: pyyaml | CPU + local I/O | — |
| `build_tanner_source_list` | Operational tooling (calibration N/A) | Yes | the official Tanner lecture sitemap and its public lecture pages (required) | stdlib | CPU + network | — |
...
| `gmail_author_pipeline` | experimental | Yes | --request (required) | req: datasketch | CPU + local I/O | — |
| `near_dup_dedup` | stable | No | A JSONL acquisition manifest (inline `text` or a resolvable `text_path`/`path` per row). (required); FILE to choose the exact-bound passage-mode checkpoint. (optional); to restore completed passage/document shards and stages from that checkpoint (or the automatic opaque temp checkpoint). (optional) | req: datasketch | CPU + local I/O | — |
| `nonprose_sweep` | Heuristic (uncalibrated) | No | B2 descriptor and referenced UTF-8 documents (required) | stdlib | CPU / stdlib | — |
...
| `passage_remediation` | Heuristic (uncalibrated) | No | owner-private root containing every bound member. (required); private descriptor for a spec-36 passage inventory and projection receipt. (required) | stdlib | CPU / stdlib (+ optional spaCy model) | — |
...
**Readiness legend.**
- **Operational tooling (calibration N/A)** — Non-inferential substrate or operator tooling. It may be user-facing, but it does not emit a thresholded claim requiring calibration.

...
- `verbatim_mosaic_audit` (`set_level_diversity`) — a pool-dependent mosaic profile of greedy verbatim span coverage, number of attributed sources, uncovered runs, and style distances at source joins against within-source controls. Uses the original DJ-Search matcher and the shipped within-document feature lens with a fixed sentence splitter and bounded vocabulary. It is descriptive and provisional; no threshold, provenance decision, plagiarism claim, or AI/human verdict. M2 model replay is gated separately by spec 82.

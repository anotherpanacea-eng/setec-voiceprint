...
6. **Corpus-hygiene checker.** ✅ Shipped. `scripts/check_corpus.py` applies the same non-prose stripping rules used by the diagnostic scripts and reports clean/warning/fail status by file. `validation_harness.py --check-corpus` wires it in as an opt-in validation-surface preflight gate without rewriting source files. An under-review Spec 72 sibling, `scripts/nonprose_sweep.py`, adds a bounded transcript/non-prose structural screen over an explicit descriptor: VTT-any, speaker-label density, closed-lexicon disfluency density, and short-line density, plus an exact authored-residual/transcript partition. It remains local stdlib code because the value is SETEC-specific grammar, path confinement, privacy, and claim boundaries; it does not replace `check_corpus.py` or introduce a dependency, and it is not shipped until its held draft PR clears.
...
10. **Per-signal threshold calibration toolchain.** ✅ Shipped (1.10.0). `scripts/calibration/` provides three scripts: `fetch_pangram_editlens.py` (HF download with license verification + `NOTICE.md` template), `editlens_to_manifest.py` (CSV/parquet → SETEC manifest with `--inspect` schema discovery + `--preset` shortcuts), `calibrate_thresholds.py` (direction-aware FPR-target sweep with FPR-resolution check + fixed-threshold paired-bootstrap CIs). Provenance ledger at `scripts/calibration/PROVENANCE.md` + machine-readable `thresholds_calibrated.json`. New `validation_harness.collect_signal_records()` helper exposes `(label, score)` pairs for the calibrator. Threshold-encoding convention: `COMPRESSION_HEURISTICS` entries (now `ThresholdSpec` dataclass instances per 1.9.2) gain a `provenance` slug pointing at the ledger when calibrated; `provisional` flag clears in lockstep. `requirements-calibration.txt` pins `huggingface_hub` + `pyarrow` as opt-in calibration-only deps. Eight regression tests guard ledger integrity (slug format, required fields, registry↔ledger referential integrity for slug + signal_path + direction, value match) regardless of corpus presence; a ninth test re-derives thresholds from the private corpus when available. v1 covers per-signal thresholds only; band thresholds, directional-cluster consistency (`0.7`), and POS-bigram smoothing α stay heuristic. See `internal/SPEC_calibration_toolchain.md` (gitignored) for the design.

### Roadmap (in priority order)

11. **Adversarial test classes for the harness.** Unicode-layer first cut ✅ shipped: `scripts/adversarial_fixtures.py` plus public fixtures for zero-width spaces, homoglyphs, and soft hyphens, with `validation_harness.py` slicing by `adversarial_class`. Remaining adversarial classes: paraphrase attacks (DIPPER-class T5 paraphrasers; Krishna et al. NeurIPS 2023 baseline) and humanizer tools (commercial humanization services like StealthGPT, UndetectableAI, Quillbot — pre-baked smoothing-reversal pipelines). Each adversarial class should be a labeled `use: validation` slice with explicit `notes` provenance; the harness reports per-class TPR at the chosen FPR.
12. **Larger ESL test class for the validation harness.** Unblocked by item #10's `fetch_pangram_editlens.py --split nonnative_english`. A real ESL slice using `language_status: non_native_advanced` as the discriminator lets the harness report ESL-FPR and native-FPR side by side. Liang et al. (*Patterns* 2023) found 61.22% average FPR on TOEFL essays across seven detectors; that's the deployment failure mode this slice catches. The corpus is local-only; the SETEC-side artifact is a calibrated threshold per signal that bakes in the ESL constraint.
13. **Band-threshold calibration (`compression_fraction < 0.15` lightly, `< 0.40` moderately).** Composite optimization across multiple weighted signals; band call should optimize PR or some accuracy proxy at the band boundary. Different math from item #10's per-signal calibration; needs its own toolchain pass once enough per-signal thresholds carry provenance to make the composite meaningful.
14. **Directional-cluster consistency calibration.** The `0.7` cutoff in `voice_distance.py` is structural ("is the cluster moving together?"), not discriminative. Needs a labeled "voice drift vs. noise" corpus, separate methodology from per-signal calibration.
15. **Manifest schema versioning.** When the manifest becomes nested or versioned, introduce `jsonschema` for per-entry structure and keep semantic cross-entry checks in `manifest_validator.py`. This is issue #6; deferred until the schema actually grows.
16. **Stylo additional oracles.** As local stylometry features mature: `stylo::imposters` as a reference for impostor-baseline verification work. (Note: `stylo::rolling.delta` is blocked at the API level — see `references/stylometry-oracle.md` "Limitations and follow-up work"; the right path if rolling-window verification becomes load-bearing is a SETEC-internal pytest contract test rather than a cross-tool oracle pass.)

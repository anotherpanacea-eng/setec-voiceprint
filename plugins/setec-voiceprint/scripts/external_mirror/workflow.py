...
def write_workflow_md(
    *,
    ...
    lines.extend([
        ...
        "",
        "## Step 2 — Record per-family metadata (SPEC v0.2)",
        "",
        "Per SPEC_external_mirror_discrimination.md v0.2, every mirror family must declare its training cutoff, the interface used, and the orchestration-layer blinding status; every human control must declare its publication date, whether the cutoff precedes the publication, and its visibility class.",
        "",
        "Drop an `outputs/$family/family.json` for each family. Shape:",
        "",
        "```json",
        "{",
        '  "mirror_panel": {',
        '    "training_cutoff_date": "YYYY-MM-DD",',
        '    "interface": "manual_fresh_chat | codex_serial_agent | claude_code_task_tool | other",',
        '    "orchestration_layer_blinding": "isolated | partial | not_blinded",',
        '    "nominal_family": "gpt_5 (only if effective family differs from operator intent)",',
        ...
        '    "visibility_class": "low | medium | high"',
        "  }",
        "}",
        "```",
        "",
        "Use `mirror_panel` for LLM mirror families; use `control` for human-control families. Both blocks are allowed on a single family but rare. Absent file = v0.1 behavior (no metadata, no derived caveats).",
        "",
        "**Two-layer blinding standard.** Subagent context isolation (each window evaluated in a fresh chat / fresh Task) is necessary but not sufficient. The *orchestrator* (whoever or whatever spawns the subagents) must also be blinded to hypothesis-bearing material — SPEC, README, run log. If the orchestrator has read those files, record `orchestration_layer_blinding: \"partial\"` or `\"not_blinded\"`; the evidence pack will surface the corresponding caveat.",
        "",
        "**Single-pass standard inference.** The mirror model must be the standard variant of its family, not a reasoning-mode variant (GPT-5-Thinking, GPT-5-Pro, o-series, Claude Extended Thinking). Reasoning-mode predictions answer a different question and are not comparable to the single-pass calibration. Set `reasoning_mode: false`; if a reasoning-mode model was nevertheless used, set `true` and the evidence pack will emit the `reasoning_mode_variant_used` caveat.",
        "",
        "**Web search disabled.** Disable the mirror model's web-search / browse / retrieval tools before the run. A search-enabled model may retrieve the target document instead of distributionally continuing it. Verify the response shows no search citations, footnoted URLs, or \"I searched for…\" language; regenerate any window where it does. Set `web_search_enabled: false`. Leaving the field absent or setting it to `true` emits the `web_search_not_disabled` caveat.",
        "",
        "**Effective vs. nominal model.** When the orchestrator is itself a frontier model (Codex, Claude Code), the subagents it spawns may inherit the orchestrator's parent model rather than the operator-intended mirror family. Record the *effective* model in the family directory name and the operator-intended family in `nominal_family` when they differ. The evidence pack will emit the `effective_model_differs_from_nominal` caveat.",
        "",
        "## Step 3 — Provide target continuation",
        "",

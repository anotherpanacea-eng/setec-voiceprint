...
def test_stale_manifest_fingerprint_caught_by_drift_gate(tmp_path):
    # recorded-field-without-verifier: a manifest's flags were produced under ITS prompt, so the drift
    ...
    current = fallacy_judge.fingerprint_prompt()
    stale = "stale" + "0" * 59
    assert stale != current
    manifest = tmp_path / "flags.json"
    manifest.write_text(json.dumps({
        "values": {"flags": [
            {"candidate_type": "faulty_generalization", "paragraph_index": 0,
             "span_text": "Everyone already knows the policy works",
             "reconstruction": "m"},
        ]},
...
def _host_transport(fn):
    ...
    jb = fallacy_judge.judge_backends
...
def test_agent_host_build_judge_resolves_without_model():
    assert "agent_host" in fallacy_judge.judge_backends.PROVIDERS
    with _host_transport(lambda req: json.dumps({"flags": []})):
        j = fallacy_judge.build_judge("agent_host", model=None)
        assert callable(j)
        res = j(SAMPLE.split("\n\n"))
    ...
    with pytest.raises(fallacy_judge.JudgeError):
        fallacy_judge.build_judge("anthropic", model=None)  # gate unchanged


def test_agent_host_in_audit_choices_default_unchanged():
    parser = fallacy_scan.build_arg_parser()

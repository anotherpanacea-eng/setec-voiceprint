...
WHOLE text passed to ``audit_phraseology`` (this surface does not run ``strip_non_prose``). An earlier
``_tokenize`` fingerprint folded case and dropped punctuation and would OVER-EXCLUDE a baseline
differing only in those — which the punctuation-/case-sensitive slot-frame templates (literal ``,``/``;``,
capitalized ``What``) score differently — silently changing the reference corpus. The whole-text hash
drops only an EXACT copy and KEEPS any baseline the audit scores differently.
"""

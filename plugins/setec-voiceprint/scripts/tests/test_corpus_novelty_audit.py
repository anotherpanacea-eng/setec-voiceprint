...
def _marked_manifest_cna(tmp_path, name="marked.jsonl"):
    p = tmp_path / name
    rows = [
...
def _clean_manifest_cna(tmp_path, name="clean.jsonl"):
    p = tmp_path / name
    rows = [
...
def test_pool_guard_refuses_a_passage_deduped_manifest(tmp_path):
    m = _marked_manifest_cna(tmp_path)
...
def test_pool_guard_does_not_fire_on_a_clean_manifest(tmp_path):
    m = _clean_manifest_cna(tmp_path)

...
def _marked_manifest_ha(tmp_path, name="marked.jsonl"):
    p = tmp_path / name
    rows = [
...
def _clean_manifest_ha(tmp_path, name="clean.jsonl"):
    p = tmp_path / name
    rows = [
...
def test_pool_guard_refuses_a_passage_deduped_manifest(tmp_path):
    m = _marked_manifest_ha(tmp_path)

...
def test_seed_preserves_other_top_level_keys(tmp_path, monkeypatch):
    """tools/check_layering.py owns a second top-level key
    (`layer_exemptions:`) in the SAME committed file, per
    specs/svp-packaging-conversion.md §4 ("Layer exemptions use the
    same migration-exemptions file"). A naive --seed here that
    overwrote the whole document would silently destroy that section
    the next time anyone regenerates the anchor exemptions -- this
    pins the merge-preserving fix that prevents that."""
    scripts = tmp_path / "scripts"
    scripts.mkdir()
    (scripts / "an_audit.py").write_text(
        "from pathlib import Path\n"
    ...
    assert len(doc["exemptions"]) == 1  # regenerated by THIS tool's seed
    assert doc["layer_exemptions"] == [{
        "from_path": "a.py", "to_path": "b.py", "edge_kind": "l2_to_l2",
    }]  # untouched -- owned by check_layering.py, not this tool

...
def _guard_target(tmp_path):
    t = tmp_path / "guard_target.txt"
    t.write_text(" ".join(_GUARD_TEXTS[:3]), encoding="utf-8")
    return t


def test_pool_guard_refuses_a_passage_deduped_manifest(tmp_path):
    m = _marked_manifest_cdnp(tmp_path)
    rc, env = _envelope(["--target", str(_guard_target(tmp_path)), "--reference-manifest", str(m), "--json"])
    assert rc == 3
    assert env["available"] is False
    assert env["reason_category"] == "bad_input"
...
def test_pool_guard_does_not_fire_on_a_clean_manifest(tmp_path):
    m = _clean_manifest_cdnp(tmp_path)
    _rc, env = _envelope(["--target", str(_guard_target(tmp_path)), "--reference-manifest", str(m), "--json"])

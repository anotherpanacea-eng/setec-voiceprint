...
def validate_params(
    *,
    ...
    unknown hash scheme, a non-positive prefix-h window, or T below the
    hard minimum. The CLI surfaces these as ``bad_input`` non-zero exits."""
    if not key:
...
DEFAULT_LICENSES = (
    "The KGW green-list z-statistic and p-value of the target under the "
    "OPERATOR-SUPPLIED scheme parameters (key, gamma, hash_scheme, vocab), "
    "plus a non-secret key_id and a PROVISIONAL detection-power band — i.e. "
    "whether the green-token count is statistically improbable under the "
    "null for THIS named scheme (Kirchenbauer et al. 2301.10226). A positive "
    "is watermark-consistent with the named scheme; the band names evidence "
    "STRENGTH for that scheme, not a class. The z / p_value / green_fraction "
    "are reported as raw values; no threshold is shipped."
)
...
def probe(
    ...
    *,
    ...
    # Validate scheme params first (cheap, key-independent).
    validate_params(

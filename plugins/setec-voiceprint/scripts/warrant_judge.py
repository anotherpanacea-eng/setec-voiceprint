    ...
    def _run(paragraphs: list[str]) -> JudgeResult:
        return JudgeResult(
            ...
            judge_identity={
                ...
                # Propagate the manifest's OWN prompt fingerprint, never discard it: the coverage was
                # produced under THAT prompt, so the drift gate must see it (else a stale manifest
                # passes a current-vs-current check). None when the manifest declared none.

...
def _effective_judge_fingerprint(args: argparse.Namespace, current_fp: str) -> str | None:
    """The prompt fingerprint that PRODUCED the coverage. For an API/mock judge that is the current
    code's prompt (``current_fp``); for a ``manifest`` judge it is the fingerprint the manifest was
    generated under (read from its ``judge_identity``), so the drift gate checks the coverage's real
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Check the operator's expected fingerprint against the prompt that actually PRODUCED the coverage —
    ...
    results = build_results(
        ...
        # Report the EFFECTIVE fingerprint that produced the coverage (== the drift-gate value): the

...
Also adds `scripts/calibration/fetch_pan24_voightkampff.py`, a local-only fetcher for the PAN@CLEF 2024 Voight-Kampff bootstrap corpus (Zenodo `10.5281/zenodo.10718757`). It follows `fetch_pangram_editlens.py`: downloads into the gitignored `ai-prose-baselines-private/` tree (resolved relative to the repo root, overridable with `--dest`), writes a `NOTICE.md` recording the research-use-only / no-redistribution terms, and refuses archive members that would extract outside the destination. No corpus material is committed.

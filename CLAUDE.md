...
- **`AGENTS.md` § Fleet / cross-repo context** — this is the **producer** of the SETEC normalized-entrypoint contract (`setec run <surface> --json`) that apodictic and setec-voicewright consume under a pinned tag + drift gate. **Changing a consumed surface ripples downstream** — follow the surface-addition checklist (`capabilities.d` fragment + golden + `_golden_*` count bumps; keep `references/contract_fixtures/` in sync).

...
All notable changes to this project. Format follows [Keep a Changelog](https://keepachangelog.com/) with [Semantic Versioning](https://semver.org/). Unreleased changes accumulate as fragments in [`changelog.d/`](changelog.d/); a release cuts them into a version section with `tools/assemble_changelog.py` and bumps the version field in `plugins/setec-voiceprint/.claude-plugin/plugin.json` (`feat:` → MINOR, `fix:` / `chore:` / `docs:` → PATCH; major is reserved for breaking changes to the public CLI / JSON contract).
...
**`check_corpus` is now recoverable at corpus scale (`--records-cache` + resume).** The single-process hygiene loop previously rescored from file 1 on any crash/host hang (the #133 belt/suspenders/buttons gap for this surface). It now accepts `--records-cache PATH`: scored per-file records are flushed atomically (tmp + `os.replace`) every `--records-cache-flush-every` files (default 200) with a `status: in_progress` → `complete` marker, progress is logged to stderr, and a re-run **skips already-scored files** and resumes from the last flush. A compat-meta gate (strip rules/aggressive + collect-stripped + warn/fail thresholds + cache version) recomputes rather than silently reusing a cache produced under different settings; `--refresh-records-cache` forces a full rescore. **Reuse is gated on a per-file content fingerprint (SHA-256):** a cached record is served only when the file still exists and its bytes are unchanged — a path+settings match alone is not safe for a hygiene gate, since a changed file could otherwise let a stale "clean" record mask newly-contaminated input. Behavior is unchanged when the flag is absent (no cache, no progress — the existing fast path). The sharded `corpus_hygiene` task via `shard_runner` remains the option for RAID-scale; this gives the default single-process path real recovery for mid-size corpora.
...
**README now documents the decision-audit consumer surfaces.** The "five task surfaces" taxonomy describes the headline laptop *workflows*; the `narrative_decision_audit` (StoryScope) and `argument_decision_audit` (ArgScope) surfaces are a different category — `consumers: [apodictic]` JSON handoffs that score structural narrative/argument decisions against a paper's human/LLM means. Adds a short "Decision-audit surfaces (consumer handoff)" subsection so the README reflects them without conflating them with the five user workflows. Docs only.
...
**`edit_magnitude_audit.py` — clean-room EditLens-style edit-magnitude scaffold (spec 13).** A new `edit_magnitude` discrimination surface that reframes ill-posed AI-edit span detection as document-level *magnitude* regression (Thai et al., EditLens, ICLR 2026). Ships as a **same-corpus calibrated estimate with the OOD-collapse caveat load-bearing — explicitly NOT an absolute "% AI" gauge**. Uncalibrated by default (`results` carry a score with no band + a "no calibrated model supplied" caveat); a calibrated model is operator-supplied via `--model` and licenses an estimate *only* for the corpus it was calibrated on. The claim-license refuses an absolute %-AI / dosage claim, cross-corpus generalization, and per-sentence localization. **License: clean-room** — EditLens weights AND dataset are CC BY-NC-SA and are NOT vendored; only the published method (RoBERTa-family regressor + MSE against a BERTScore-style pre/post similarity proxy) is reimplemented, with the base model (roberta-large, MIT) and a non-NC paired corpus as operator-supplied gating prerequisites. Inference + a sibling clean-room `calibration/train_edit_magnitude.py` **scaffold** — its orchestration, provenance writer, and license gate are real and smoke-tested on synthetic pairs, but the HF training loop is **intentionally unimplemented**: `default_train_model` exits with a clear "not wired" error until an operator implements it (or injects `train_model`), so the CLI does not yet produce a checkpoint and the audit stays uncalibrated until one is supplied. The model path is injectable so **no model loads in the build or tests**. Adds `edit_magnitude` to `output_schema.VALID_TASK_SURFACES` + `claim_license.TASK_SURFACE_LABELS` (additive). 14 tests in `test_edit_magnitude_audit.py`.

_Tagged `feat` (→ MINOR). The `plugin.json` version bump is applied at merge, not pinned here — open PRs merge in an unknown order._

**`rewriting_invariance_audit.py` — Raidar-style rewrite-invariance discrimination evidence (Surface 6 / discrimination).** Implements `specs/15-raidar-rewriting-invariance.md` on a new `rewriting_invariance` task surface. The Raidar idea (Mao et al., ICLR 2024): prompt an LLM to "rewrite to improve clarity," then measure how much it changed the text (character edit-distance via stdlib `difflib` + token-overlap distance) averaged over N trials — LLMs edit their own AI-like prose *less* than human prose, so low distance ⇒ more AI-like, high ⇒ more human-like. A paraphrase-robust axis orthogonal to perplexity. **Clean-room implementation of the published idea — no Raidar repo code, prompts, or data were consulted or vendored** (the rewrite prompt and distance metrics here are our own). The rewrite client is **injectable** (mirrors `narrative_judge.py`'s pluggable-client + recorded-provenance pattern): `audit_rewriting_invariance` takes a `rewrite_fn`, the CLI builds an API-backed reference backend from `--judge MODEL`, and the test suite passes a deterministic stub rewriter so **no real LLM call is made in tests**. Provenance pins the judge model + the exact rewrite prompt + its SHA-256 fingerprint, because the distance is meaningless without them. **Uncalibrated by design**: emits the mean rewrite-distance + per-trial distances + provenance only — no band, no verdict, no shipped threshold; the claim-license licenses "the mean rewriting distance under judge model M and prompt P," refuses any AI/human verdict absent operator thresholds, and flags the strong model+prompt dependence and the per-call LLM cost. Adds `rewriting_invariance` to `output_schema.VALID_TASK_SURFACES` + `claim_license.TASK_SURFACE_LABELS` (additive). 10 tests in `test_rewriting_invariance_audit.py` (`rewriting_invariance_audit`).

_Tagged `feat` (→ MINOR). The `plugin.json` version bump is applied at merge, not pinned here — open PRs merge in an unknown order._

**`intrinsic_dimension_audit.py` — clean-room PHD intrinsic-dimension discrimination evidence (Surfaces / discrimination-topology).** A new research-grade audit on a new `intrinsic_dimension` task surface (`specs/14-intrinsic-dimension-phd.md`): the persistent-homology dimension (PHD) of a text's contextual-embedding point cloud — the single most *orthogonal* discrimination axis to perplexity / lexical signals in the literature (Tulchinskii et al. 2023, arXiv:2306.04723), with human prose tending to a higher intrinsic dimension than AI prose. **Clean-room** decision: the upstream GPTID reference implementation is **not vendored** (license unconfirmed; the PHD math is published), and **no external TDA library** (`ripser` / `gph`) is added — instead the H0 persistence diagram is computed via the exact equivalence "H0 death-times = Euclidean minimum-spanning-tree edge weights," using `scipy.sparse.csgraph.minimum_spanning_tree`; PHD is read off the log-log scaling of the H0-persistence functional vs. seeded sub-sample size. The embedder is **injectable** (`audit(embed=...)`): tests pass a deterministic stub, the runtime path constructs an Apache/MIT `EmbeddingBackend` lazily inside `main()` — **no model is loaded at import or in the build/tests**. Ships **uncalibrated**: emits the scalar PHD + point count + embedding-model id, with **no band and no threshold**, and the claim-license refuses an AI/human verdict absent operator-supplied thresholds (noting short-text instability and embedding-model dependence). Adds `intrinsic_dimension` to `output_schema.VALID_TASK_SURFACES` + `claim_license.TASK_SURFACE_LABELS` (additive). 6 tests in `test_intrinsic_dimension_audit.py`.

_Tagged `feat` (→ MINOR). The `plugin.json` version bump is applied at merge, not pinned here — open PRs merge in an unknown order._
**`productive_roughness_audit.py` — strictly baseline-relative productive-roughness deviation (spec 10).** A new `productive_roughness` voice-coherence surface measuring how a draft's sentence-internal roughness — fragment rate (spaCy dependency parse: no finite/root verb), sentence-initial coordinating-conjunction rate, contraction rate, adjacent-word-repetition rate, interjection/aside rate, very-short-sentence rate — deviates from *the writer's own baseline*, reporting each feature as `{draft, baseline_mean, baseline_sd, z}`. The roadmap's hard constraint is frontloaded in code: **`--baseline-dir` is REQUIRED** (the audit refuses to run on a single document), and the claim-license refuses any absolute "too smooth/too rough" or quality judgment, any voice/authorship/AI verdict, and any use without a writer-specific baseline — it measures deviation, never an absolute. Adds `productive_roughness` to `output_schema.VALID_TASK_SURFACES` + `claim_license.TASK_SURFACE_LABELS` (additive). 12 tests in `test_productive_roughness_audit.py`.

_Tagged `feat` (→ MINOR). The `plugin.json` version bump is applied at merge, not pinned here — open PRs merge in an unknown order._

**`dialogue_voice_audit.py` — per-character dialogue-voice profiling + cross-character divergence (Surfaces Tier 4, Round 2 of `pov_voice_profile`).** Implements `specs/11-dialogue-voice-audit.md` on the EXISTING `voice_coherence` task surface (adds no new surface). Where `pov_voice_profile` profiles per-POV *narration*, this audit profiles per-character *dialogue* — character-voice collapse surfaces in spoken lines before narration. Method: extract quoted spans, tag-attribute them to speakers via "said X" / "X asked" dialogue tags (unattributed dialogue is bucketed under `<unattributed>`, profiled separately, and NEVER force-attributed — it is not a coreference engine), then per character compute contraction rate, mean turn length + variance, dialogue-tag verb diversity, vocative rate, interruption/trailing-punctuation rate (— / …), and top function words + discourse markers. Reports a cross-character pairwise **divergence matrix** + a `converged_pairs` list that is explicitly **descriptive, not a verdict** — low divergence may be intentional (shared milieu / chorus) or symptomatic (collapse); no threshold, no flag. Optional `--baseline-dir`. spaCy-backed (`en_core_web_sm`) with graceful degradation (`available: false` + actionable warning, never a traceback) when spaCy/model is missing. Claim-license refuses author-identity / AI-provenance / quality inference and notes attribution is tag-heuristic. 10 tests in `test_dialogue_voice_audit.py`.
...
**`narratorial_distance_audit.py` — descriptive narratorial-distance / FID profile (Surfaces Tier 4).** A new audit on a new `narratorial_distance` task surface (voice-coherence family) implementing `specs/12-narratorial-distance-audit.md`. For developmental editing of literary fiction, it measures *where the narration sits* relative to a character's consciousness — close (free-indirect discourse) vs. distant — per window/chapter: pronoun anchoring (3rd-person + proximal), perception/cognition-verb density (default lemma set, `--verb-lexicon` to override), deictic anchoring (proximal `here/now/this` vs. distal `there/then`), evaluative-adjective density, and a heuristic FID score (past-tense + proximal-deixis + perception + no quotation/tag). Reports per-window features, an overall close/distant distribution, and a **distance trajectory** across document position (`--window-strategy paragraph|chapter`). spaCy POS/dep + lemma; graceful `available=False` (never a traceback) when spaCy / `en_core_web_sm` is missing. Descriptive **only** — no band, no verdict: the claim-license refuses authorship / AI / writing-quality inference and notes FID detection is a heuristic signal, not a parse of literary intent. Adds `narratorial_distance` to `output_schema.VALID_TASK_SURFACES` + `claim_license.TASK_SURFACE_LABELS` (additive — the surface-parity test asserts only a subset). 10 tests in `test_narratorial_distance_audit.py`.
_Tagged `feat` (→ MINOR). The `plugin.json` version bump is applied at merge, not pinned here — open PRs merge in an unknown order._
...
**`voice_fingerprint` — learned style-embedding authorship-consistency surface (spec 02).** A new `authorship_embedding` surface (sibling to `voice_distance`, not a replacement): embeds windows with a frozen LUAR style encoder (Apache-2.0) and reports the cosine-similarity *distribution* — internal voice consistency (single), consistency vs a baseline voice centroid (two-corpus), or where a target's candidate-similarity falls relative to an impostor pool (n-way, descriptive only). Refuses same-person / different-author / AI–human verdicts; ships PROVISIONAL (no threshold) under "Stylometry to the people." The brief's #1-ranked addition (orthogonal axis, cleanest license, low effort).

- **New surface + CLI.** `voice_fingerprint.py TARGET [--baseline-dir DIR] [--impostor-dir DIR] [--model luar|wegmann] [--window-strategy paragraph|fixed-token] [--window-size N] [--device DEVICE] [--json] [--out PATH]`. Reuses `semantic_trajectory_audit`'s windowing (no reimplementation); emits the `build_output(task_surface="authorship_embedding")` envelope — `results.cosine_distribution {mean, sd, min, p10, p50, p90}` plus `per_window`, and in n-way `target_vs_candidate` / `target_vs_impostors` — carrying a no-verdict `ClaimLicense` (LUAR register-skew, short-text fragility, cross-model incomparability caveats).
- **LUAR is an episode model.** Loaded via `transformers` `AutoModel ... trust_remote_code=True` (needs `einops`); inputs are reshaped to `(N, 1, seq)` and LUAR returns the pooled author embedding directly. ~0.5–1.4 GB download; CPU-feasible, GPU optional (<2 GB VRAM). Wegmann (`AnnaWegmann/Style-Embedding`) opt-in via `sentence-transformers`. Encoder is injectable (`_load_encoder`) so tests stub it deterministically.
- **Registrations.** `authorship_embedding` added to `output_schema.VALID_TASK_SURFACES` and `claim_license.TASK_SURFACE_LABELS`; `voice_fingerprint` entry added to `capabilities.yaml` (`status: empirically_oriented`, `handoff: experimental`); `dependency_check` gains a `style_embedding` optional-tier note.
- **8 spec tests** (`test_voice_fingerprint.py`) pass against a deterministic stub encoder; the real LUAR path was verified end-to-end on the GPU box (CPU device, 23 windows → mean cosine 0.63).
- **Cross-platform drift-linter fix.** `tools/check_capabilities_drift.py` Check-1 normalizes the source path with `.as_posix()` — it had rendered every script as an `orphan_script` on Windows (backslash vs forward-slash `script_path` comparison), failing the repo's own drift test on a clean tree.
**New Surface-5 signal: Fast-DetectGPT conditional-curvature detector (`fast_detect_curvature`).** Adds a zero-shot conditional-probability *curvature* statistic (Bao et al., Fast-DetectGPT, ICLR 2024; MIT) as discrimination evidence genuinely distinct from Binoculars' cross-perplexity ratio and DivEye's surprisal moments. Where Binoculars asks "how much more predictable is x under a scorer than under an observer?" (a model *pair*, a ratio), Fast-DetectGPT asks "does x sit at a local *peak* of one model's conditional probability surface?" (a single model, a z-score).

- **New `discrimination_curvature` task surface.** A discrete sibling to `binoculars_discrimination`, registered in `output_schema.VALID_TASK_SURFACES` and `claim_license.TASK_SURFACE_LABELS`. The spec's Open Question floats folding Binoculars + curvature + DivEye under one shared `discrimination_evidence` surface; that refactor (which would touch `binoculars_audit`'s tag) is intentionally **not** done here — it's left as a maintainer decision so this PR stays scoped to the new signal.
- **`fast_detect_curvature.py` CLI.** `fast_detect_curvature.py TARGET [--model ALIAS] [--n-samples N] [--surprisal-dtype auto|fp32|fp16|bf16] [--device DEVICE] [--seed SEED] [--per-position] [--json] [--out PATH]`. Envelope `results` carry `model_id`, `curvature_score`, `n_samples`, `n_tokens`, and an optional `per_position` series. The curvature is the standardized gap (z-score) between the text's total conditional log-prob and the mean/std of conditional log-probs for alternative tokens sampled, seeded, from the model's own per-position conditional distributions: `curvature = (Σ lp_t − Σ μ_t) / sqrt(Σ var_t)`.
- **Uncalibrated by default — no verdict, no shipped threshold.** Mirrors `binoculars_audit`'s posture exactly: the audit emits the raw z-score and a `no_calibrated_thresholds_supplied` caveat, never a band. The `ClaimLicense` block licenses "the conditional-curvature statistic under model M is C" and refuses any AI/human label absent operator-supplied thresholds, noting the in-distribution caveat and paraphrase sensitivity from the brief.
- **Reuses `surprisal_backend` for model loading.** No new model wrapper. The real-model scoring path (`score_curvature_with_backend`) reads per-position conditional log-prob distributions from `SurprisalBackend.score_text_with_distributions` and is injectable via an `audit(..., score_fn=...)` hook so tests run against a deterministic stub — no real model load, no GPU in CI. Single model by construction (no observer); the statistic never touches a Binoculars or surprisal field.
- **7 named tests** in `test_fast_detect_curvature.py` (`test_envelope_shape`, `test_no_default_threshold_no_band`, `test_claim_license_refuses_verdict`, `test_curvature_deterministic_with_seed`, `test_orthogonal_statistic`, `test_missing_torch_graceful`, `test_capabilities_entry_present`), plus closed-form-math and degenerate-variance helper tests. The capabilities manifest gains a `fast_detect_curvature` entry (`surface: discrimination_curvature`, `status: literature_anchored`, `handoff: experimental`, `compute.tier: surprisal`, `length_floor_words: 50`, deps `transformers` + `torch`).
- **Drift linter Windows path fix.** `tools/check_capabilities_drift.py` compared `str(Path.relative_to(...))` against the manifest's forward-slash `script_path` keys; on a Windows checkout `str(Path)` yields backslashes, so *every* script looked like an orphan. Switched to `.as_posix()` — a no-op on POSIX, correct on Windows.
**`pan_replay` — PAN@CLEF obfuscation-replay robustness card (spec 04).** The brief's #3 near-term, license-clean addition: PAN names the open problem as OOD generalization, short texts, and obfuscation robustness — exactly SETEC's adversarial territory. SETEC already had homoglyph / zero-width / paraphrase fixtures (`adversarial_fixtures.py`) and a per-signal robustness card (`adversarial_robustness_card.py`); what was missing was a harness that *systematizes* the two against the standard public benchmark, so SETEC can state, per signal, "stable under X, collapses under Y" against the same fixtures the field uses. `pan_replay` is that harness — and it is **orchestration over existing components**, not a new detector. It lives on the existing `validation` task surface (no new surface added).

- **`plugins/setec-voiceprint/scripts/calibration/pan_replay.py`** — CLI `pan_replay.py --fixtures DIR [--classes unicode,paraphrase,lang_switch,short] [--signals ...] [--json] [--out PATH]`. Loads (clean, obfuscated) pairs from a `pairs.jsonl` manifest in `--fixtures DIR`, scores each text with the *same* `variance_audit.audit_text` + `classify_compression` path the validation harness runs per entry (CPU-only Tier 1–3; no model loads), and reuses `adversarial_robustness_card.build_robustness_card` to produce per-(signal × obfuscation-class) cells. Each cell carries the clean→obfuscated relative change Δ and a stable / degraded / collapsed tag (mapped from the reused card's label vocabulary). Per-class slicing is strict — each obfuscation class is scored on its own pairs only, no cross-class mixing. **No aggregate robustness or accuracy score is emitted**, and the `ClaimLicense` block refuses any detector-accuracy headline while stating fixture provenance.
- **Fixture provenance / no PAN redistribution.** PAN@CLEF data redistribution is gated, so the harness does **not** vendor real PAN data. A tiny *synthetic* fixture (`plugins/setec-voiceprint/scripts/test_data/pan_replay_fixture/`) exercises the orchestration across all four classes — original prose authored for the fixture, with unicode variants generated deterministically via `adversarial_fixtures.py` and paraphrase / lang_switch / short variants hand-written; it is explicitly NOT PAN content. Bringing the real fixtures is a follow-up: a PAN-account-gated, local-only fetcher modeled on `fetch_pangram_editlens.py` (download locally, write a NOTICE.md with attribution + redistribution prohibition, never commit the corpus). A missing `--fixtures` dir (or a dir with no `pairs.jsonl`) yields a clear error and exit 2, not a traceback.
- **`capabilities.yaml`** gains a curated `pan_replay` entry: `surface: validation`, `status: empirically_oriented`, `handoff: internal`, `compute.tier: core`, `dependencies.python: []` (reuses the harness's optional spaCy / sklearn for Tier 2–3 with the same graceful degradation). Inputs = the fixture dir.
- **6 spec tests** (`plugins/setec-voiceprint/scripts/tests/test_pan_replay.py`): `test_envelope_and_surface`, `test_per_class_slicing`, `test_refuses_aggregate_score`, `test_robustness_card_reuse`, `test_missing_fixtures_clear_error`, `test_capabilities_entry_present`.
- **Drift-linter Windows fix.** `tools/check_capabilities_drift.py` Check-1 now normalizes the source path with `.as_posix()` before comparing against the manifest's forward-slash `script_path` values — without it, `relative_to(REPO_ROOT)` yields backslashes on Windows and flags every script as an orphan. No-op on POSIX; identical to the fix in PRs #156/#157.
**ESL / L2 / translated-text fairness slice in `validation_harness` (spec 05-esl-fairness-slice).** SETEC has preached the non-native-English false-positive problem since its "why no verdict" posture — Liang et al. (*Patterns* 2023) flagged 61% of human TOEFL essays as AI — and it already validates a `language_status` field on every manifest entry. But the validation harness reported a single pooled FPR, quietly averaging native and non-native rates into one number that hides the exact failure the field is most embarrassed by. This extends the existing harness (it is not a new script and not a new task_surface — it stays `validation`) so the harness now *measures* that failure mode per language background and *refuses to pool it away*.

- **`language_status_slices` block.** The harness report gains a dedicated per-`language_status` block carrying the spec contract shape `{n, fpr, tpr, roc_auc, ci}` for each status present (`native` / `non_native_advanced` / `non_native_intermediate` / `learner` / `unknown`, the vocabulary already validated by `manifest_validator`), alongside the existing surface × register × length × ai-status slices. The `ci` is the ROC-AUC paired-bootstrap CI (reusing the harness's existing per-slice bootstrap-CI checkpoint plumbing with a per-status derived seed); per-rate FPR/TPR carry their own Wilson/Clopper-Pearson proportion intervals inside their rate dicts. Each non-native status is flagged `is_non_native: true`.
- **`--slice-by language_status`, on by default.** The slice is automatically active when any entry carries a non-`unknown` `language_status`. `--slice-by language_status` forces it on even for an all-`unknown` manifest; `--no-language-status-slice` forces it off (and is what trips the don't-pool refusal on a mixed corpus).
- **"Don't pool" guard.** When native and non-native entries are both present and slicing is disabled, the harness REFUSES to emit a single aggregate FPR — it replaces the pooled `rates.fpr` in the overall threshold metrics with an explicit refusal marker (`value: null`, `refused: true`) and annotates the operating point, rather than publishing a number that buries the non-native rate inside the native one. When only native (or unlabeled) entries are present, the aggregate FPR is annotated **native-only** with an explicit caveat that it says nothing about SETEC's false-positive rate on non-native prose.
- **Empty/underpowered → explicit caveat, never a silent 0.** A `language_status` slice with zero scored controls (negatives) reports `fpr: null` plus an `EMPTY/UNDERPOWERED … not 0` caveat — SETEC will not let a missing L2 control set read as "no false positives on non-native prose." A slice with fewer than `MIN_LANGUAGE_SLICE_CLASS_N` (5) controls reports the point estimate but marks it `fpr_powered: false` with a wide-CI caveat. The same logic applies to TPR via positives.
- **Claim license.** A new `claim_license.language_status_slice` clause licenses per-status performance only where the control class is powered, and explicitly REFUSES evaluative or disciplinary use of underpowered/empty slices and of the pooled aggregate when native + non-native are mixed without slicing. When no non-native entries are present at all, the clause licenses *nothing* about the documented failure mode.
- **Synthetic test fixture (`scripts/test_data/esl_fairness_fixture/`).** Seven small hand-written prose files carrying native/non-native/learner language statuses so the unit tests can pin the slice's *logic* (shape, guard, caveats, license) without shipping external corpora. These are explicitly NOT operator-sourced and license no empirical ESL false-positive rate; sourcing a real operator/public-domain L2/translated corpus is the spec's documented gating follow-up.
- **`scripts/tests/test_validation_harness_esl_slice.py`** — the five named spec tests (`test_language_status_slices_present`, `test_refuses_pooled_fpr`, `test_empty_slice_caveat`, `test_native_only_annotation`, `test_backward_compat`) plus a companion pinning that the same mixed manifest with the slice *on* slices instead of refusing. Backward compatibility is load-bearing: `test_backward_compat` pins that a manifest with no `language_status` produces the exact prior report shape (slice off, aggregate FPR unmodified, all prior slice families present, no annotation/refusal keys), since `validation_harness` is a `handoff: stable` surface consumed by APODICTIC. The `validation_harness` capability's `use_when` is updated to mention the ESL slice; no new manifest entry, so no new readiness row.
...
- **Seeder updated to v0.2.0 schema.** Pre-fix, `tools/seed_capabilities.py` still rendered `schema_version: "0.1.0"` and emitted only `dependencies.python`, so the natural follow-up workflow (linter says orphan_script → operator runs the seeder → seeder regenerates with stale shape → linter still passes silently → availability false-negatives come back) silently regressed the v0.2 fix. The seeder now emits `schema_version: "0.2.0"` plus `python_optional: []` on every entry as a hint to operators to demote graceful-degradation deps during hand-curation. The header-comment block documents the new vocabulary.
- **Drift linter catches duplicate `script_path`.** Pre-fix, `check_drift()` only checked duplicate `id`s; two entries with different ids but the same script_path silently collided in `by_script_path[sp] = entry`, dropping whichever lost the overwrite race from every downstream check. The manifest is one-source-of-truth per script, so duplicate script_paths are themselves drift. New `duplicate_script_path` violation kind; the index now ignores the second claim (first wins) so downstream checks still run on a defined entry.

### Added (review followups)

- **`plugins/setec-voiceprint/scripts/tests/test_seed_capabilities.py`** — 4 tests for the seeder: schema version pin (regression for the silent stale-shape regenerate path), `python_optional` always emitted, end-to-end parse + minimum entry count, status: todo invariant on seeded entries.
- **`plugins/setec-voiceprint/scripts/tests/test_check_capabilities_drift.py`** — 1 new test (`test_duplicate_script_path_detected`) pinning the new violation kind and that it fires independently of `duplicate_id`.

Plugin version 1.108.1 → 1.108.2.

**Capabilities manifest v0.2 + polarity-audit perf fix (self-review followups).** Two issues surfaced by the in-session self-review of PR #129 fixed before merge:

- **`--available` no longer false-negatives on graceful-degradation audits.** Pre-fix, `variance_audit` listed `[nltk, numpy, sentence_transformers, sklearn, spacy, textstat]` under `dependencies.python`, so on a stdlib-only machine `capabilities.py list --available` would hide it — even though its Tier 1 primary path runs without any of those. Schema bumped to v0.2.0 with a new `dependencies.python_optional` field for graceful-degradation deps (sentence_transformers → TF-IDF fallback; textstat → stdlib FKGL approximation; NLTK → regex tokenization; etc.). The `--available` filter gates only on required deps; optional deps surface in `show` output as "missing optional deps (graceful degradation): …" so operators see what they're missing without losing the audit from the list. All 10 curated entries re-classified (variance_audit / voice_distance / aic_pattern_audit / dependency_check / narrative_decision_audit / restoration_packet / manifest_validator now have empty required-dep lists; idiolect_detector requires scipy only; binoculars_audit requires transformers + torch only; validation_harness requires sklearn + statsmodels only). `entry_available()` now returns a 3-tuple `(available, missing_required, missing_optional)`.
- **`narrative_polarity_audit.per_signal_polarity` perf fix: O(S²·N) → O(S·N).** Pre-fix, the inner `for feat → for sig → for row` loop called `per_signal_contributions(r.values)` redundantly — recomputing all 33 signal contributions for every (signal × row) pair. New `_index_target_values()` helper pivots each row's contributions into a `{(feat_key, option): target_value}` dict once per row, eliminating ~33× redundant work. On EditLens-sized manifests (≈1,506 rows) that's ~50k contribution objects instead of ~1.6M. New `test_per_signal_polarity_calls_pivot_once_per_row` pins call count via monkeypatch; `test_index_target_values_matches_per_signal_contributions` pins behavioral equivalence (the perf fix isn't allowed to drift the numbers).

### Added (discoverability)

- **`plugins/setec-voiceprint/scripts/tests/test_capabilities.py`** — 3 new tests for the v0.2 optional-deps semantics: `test_entry_available_optional_does_not_block`, `test_entry_available_mixed_required_and_optional`, `test_variance_audit_available_when_only_optional_deps_missing`. The last is the regression pin for the original bug.
- **`plugins/setec-voiceprint/scripts/tests/test_narrative_polarity_audit.py`** — 2 new tests for the perf fix: behavioral equivalence + call-count pin.

**Discoverability layer: capabilities manifest + query CLI + `/setec` skill + drift linter.** Single source of truth for what every user-facing script in SETEC does, when to use it, when not to, and what compute tier it needs. Replaces the prior "read three docs and grep the source" discovery model. Phase A ships with 10 hand-curated entries (variance_audit, voice_distance, idiolect_detector, aic_pattern_audit, binoculars_audit, narrative_decision_audit, validation_harness, restoration_packet, dependency_check, manifest_validator) + 54 auto-seeded entries for the remaining TASK_SURFACE-bearing scripts; subsequent PRs promote TODO entries off `status: todo` incrementally, gated by the drift linter.

### Added (discoverability)

- **`plugins/setec-voiceprint/capabilities.yaml`** — 64 entries, one per user-facing script. Schema documented inline; each entry carries `id`, `script_path`, `surface`, `family`, `status`, `purpose`, `use_when`, `do_not_use_when`, `inputs`, `outputs`, `compute` (tier + cost_note + length_floor_words), `registers`, `dependencies` (python + sdks_optional), `examples`, `references`.
- **`plugins/setec-voiceprint/scripts/capabilities.py`** — query CLI. Subcommands `list` (with `--surface`, `--family`, `--status`, `--tier`, `--register`, `--length-floor-max`, `--available`, `--include-todo`, `--format table|json|md|ids`), `show ID`, and `recommend --situation "<free text>"`. Recommend uses a curated routing table for top situations (essay / fiction / revision / calibration / setup / narrative / ESL) plus a use_when keyword fallback. `--available` integrates with `importlib.util.find_spec` to filter to audits whose Python deps are installed locally.
- **`plugins/setec-voiceprint/skills/setec/SKILL.md`** — `/setec` slash command. Three-step conversational flow: ask what they're trying to figure out → gather context (length / register / compute) → call `capabilities.py recommend` and present the result with pipeline framing. Documents canonical routes for the most common situations.
- **`tools/seed_capabilities.py`** — one-shot bootstrap that walks `plugins/setec-voiceprint/scripts/` for `TASK_SURFACE`-bearing files, parses each via the `ast` module, infers compute tier from imports (`anthropic|openai|google.genai` → `api_llm`; `transformers|surprisal_backend` → `surprisal`; `pypdf|ocrmypdf` → `ocr`; `requests|bs4` → `acquisition`; `huggingface_hub|pyarrow` → `calibration`; `sentence_transformers` → `optional`; else `core`), and emits a draft manifest with `status: todo` placeholders.
- **`tools/check_capabilities_drift.py`** — CI linter. Four checks: (1) orphan scripts (TASK_SURFACE-bearing source with no manifest entry), (2) orphan manifest entries (entry's script_path doesn't exist), (3) surface drift (manifest surface != source TASK_SURFACE constant), (4) TODO-content (non-todo entries whose `family` / `use_when` / `do_not_use_when` still contain TODO markers). Stdlib + PyYAML. Exit 0 clean / 1 drift / 2 internal error.
- **`plugins/setec-voiceprint/scripts/tests/test_capabilities.py`** — 23 tests covering manifest loading, filter chain composition, recommendation routing (fiction → narrative_decision_audit + aic_pattern_audit; essay → variance_audit + aic; setup → dependency_check), TODO filtering, availability detection via importlib, render functions (table / md / json / show), CLI exit codes.
- **`plugins/setec-voiceprint/scripts/tests/test_check_capabilities_drift.py`** — 8 tests covering clean-repo passage, synthetic orphan-entry / orphan-script / surface-drift / duplicate-id / todo-content detection, and `status: todo` exemption from content checks. Companion to the v0.1 Surface 6 audit. Wires the analytics layer (Stage C1 / C2 binary + 6-way XGBoost training with prompt-bootstrap CIs) and the feature-deduplication stage (B4, embedding-based single-linkage clustering matching the paper's procedure), and stubs the LLM-driven stages (A1–A3, B1–B3, B5) with a canonical pattern in `stages/a1_prompt_extraction.py` plus a `stub_template.py` operators specialize per stage. An orchestrator (`pipeline.py`) chains the stages with per-stage checkpoints + input SHA-256 verification. Spec doc enumerates three replication levels (L1 audit-only, L2 evaluation-replication, L3 full induction) with per-stage cost estimates.
...
- **`scripts/calibration/bakeoff_mage_tier34_compare.py`** — companion reader. Walks every `survey_phaseA_*.json` and `survey_phaseB_*.json` under `--surveys-dir`, extracts `direction_aware_auc` per target signal from the survey's flat `rows` list (the layout `calibration_survey.SurveyRow.to_dict` actually emits), and prints two markdown comparison tables (one per phase) plus a "recommended winner" line per phase. Polarity-inverted configs (da_AUC < 0.5) are flagged with `!`; clear-separation configs (da_AUC ≥ 0.55) are flagged with `*`. The maintainer reads the tables and picks Phase C winners (judgment call combines cost, reproducibility, deployment fit beyond raw da_AUC).

### Review fixes applied in this PR

1. **Reader path corrected** (`bakeoff_mage_tier34_compare.py::_da_auc`). The first draft probed `survey["per_signal"][sig].calibration.direction_aware_auc`, a path `calibration_survey.py` has never produced. The survey emits `rows: [{"signal": ..., "direction_aware_auc": ...}]` flat at top level (see `SurveyRow.to_dict`). The reader now iterates `survey["rows"]` and matches on `row["signal"]`. Without this fix the comparison table rendered every cell as MISSING / "no winner" regardless of actual scores.
2. **Winner selection robust across signals** (`_format_table`). The first draft ranked by `max(da_AUC)` per model, so a config like `[0.99, 0.10]` (one excellent, one polarity-inverted) beat a stable `[0.70, 0.70]`. Selection now disqualifies any config with any target signal either missing or below 0.5, then ranks survivors by the **minimum** da_AUC across the phase's target signals — the "weakest link" metric — so the recommended winner is robust across every target signal. The diagnostic "no winner" branch now names which configs were inverted vs missing data.
3. **Tests added** (`tests/test_bakeoff_mage_tier34_compare.py`, 16 cases). Cover the row-reader contract (unknown signal returns None, null value returns None, bool rejected, legacy `per_signal` layout returns None to pin the bug); winner selection (inverted sibling disqualifies high-max config, eligible configs ranked by min not max, missing signal disqualifies, no-winner diagnostic names the failure mode); `_format_da` markers.
...
- **`score_texts` batched path** (#90) gains an over-context preflight (added in code review on PR #98). After tokenization the padded batch length is checked against `max_position_embeddings`; if the batch is over-context, `SurprisalBackendError` is raised *before* the forward pass. The calibration loop (`_refill_surprisal_cache`) catches the typed error via its existing batch-failure latch, sets `batched_surprisal_disabled = True`, and falls through to the per-entry path — where the chunked `score_text` handles each row safely. This closes the operator-facing crash path (the latch's `except Exception` never fires when the model hangs without raising; the preflight ensures Python *does* raise before the forward pass reaches the GPU). Adding *within-batch* chunking (rather than fall-through) is the larger refactor and is deferred — for a corpus with any over-context rows, the fall-through to per-entry mode (where chunking is in play) is the correct cost: throughput drops only for the affected slice of the corpus, but no row hangs and no operator-visible silence occurs. The fp32 `log_softmax` upcast (#93) and right-padding defenses are untouched.

### Added

- **6 new tests** in `test_surprisal_backend.py` (52 total, was 46): `test_score_text_chunks_over_context_input` (10-token input on a 4-position model produces a 7-element concatenated series across 3 chunks), `test_score_text_short_input_unchanged_under_chunking` (single-chunk fast path), `test_score_text_chunking_falls_back_to_1024_when_config_lacks_max_position` (fallback chain), `test_score_text_top_k_positions_index_into_concatenated_series` (top-k positions are 1-indexed into the concatenated series, not the original input_ids), `test_score_texts_raises_on_over_context_batch` (preflight raises `SurprisalBackendError` before the forward pass when the padded batch exceeds `max_position_embeddings`), `test_score_texts_short_batch_unchanged_under_preflight` (in-context batches still go through the batched path normally).
...
Two of the four (`aesthetic_authority_audit`, `kicker_density`) are called as Python functions by other scripts (`variance_audit`, the aesthetic_authority compound itself). The migration preserves their function-level return shape unchanged so function-call consumers stay green; the envelope is added only on the CLI path via `build_audit_payload()`. The legacy `"claim_license": "voice_diagnostic"` tag string on the inner blocks stays in place for variance_audit and aesthetic_authority's function-level consumption; the envelope's top-level `claim_license` is the new structured 11-key dict.

### Added

- **`agency_abstraction_audit` migrated** (smoothing_diagnosis). Split `_claim_license_block` into `_claim_license` (ClaimLicense) + markdown wrapper; new `build_audit_payload` lifts signals (`raw_counts`, `densities_per_1k`, `entity_to_action_ratio`, `compression`, `baseline_comparison`) under `results`; `preprocessing` rides under `target.preprocessing`; baseline metadata routes through `build_baseline_metadata`.
- **`discourse_move_signature` migrated** (smoothing_diagnosis). Same shape as agency_abstraction; B.3 state-routed caveats via `with_state_caveats(...)` preserved. The 8 results keys (`category_counts`, `category_densities_per_1k`, `total_marker_density_per_1k`, `move_sequence`, `move_sequence_bigrams`, `move_sequence_entropy_bits`, `marked_only_entropy_bits`, `compression`) flow under `results`; `n_sentences` rides under `target_extra`; `ai_status` propagates to envelope top-level.
- **`aesthetic_authority_audit` migrated** (smoothing_diagnosis). Compound AIC-8 + AIC-9 audit; the full legacy block — including the three per-detector blocks (`aic_9_kicker_density`, `aic_8_image_conjunction`, `aic_8_prestige_metaphor`) and the `compound` co-occurrence metrics — lives under `results`. New `_claim_license` per the SPEC §11 carve-out. `target_words` pulls from the inner image-conjunction `diagnostics.total_tokens` with a text-split fallback. Register lifts into `target_extra` when set.
- **`kicker_density` migrated** (smoothing_diagnosis). The audit function `kicker_density()` keeps its existing return shape because `variance_audit.compute_aic_9_kicker` and `aesthetic_authority_audit.aesthetic_authority_audit` call it as a function and read top-level keys. The envelope is added on the CLI path only via `build_audit_payload(block, target_path, text)`. New `_claim_license` surfaces comparison_set fields from the diagnostics block.
- **58 new tests** across four files: `test_agency_abstraction_audit_schema.py` (15), `test_discourse_move_signature_schema.py` (16), `test_aesthetic_authority_audit_schema.py` (16), `test_kicker_density_schema.py` (11). Each pins envelope keys, the no-legacy-keys invariant on the envelope's top level, results-carries-script-payload, the 11-key structured claim_license, and the unavailable / baseline / register paths where relevant. The kicker_density and aesthetic_authority tests additionally guard the function-call contract (`signal_path` / `family` / `value` at the function-return top level) so cross-script consumers are pinned against future regressions.

### Changed

- **`agency_abstraction_audit` JSON output shape is a BREAKING CHANGE.** The pre-1.83 CLI output had `task_surface` / `tool` / `version` / `available` / `n_words` / `raw_counts` / `densities_per_1k` / `entity_to_action_ratio` / `compression` / `preprocessing` / `baseline_block` / `baseline_comparison` at the top level. Post-1.83 carries the schema_version 1.0 envelope: signals under `results`, preprocessing under `target`, baseline metadata under `baseline`.
- **`discourse_move_signature` JSON output shape is a BREAKING CHANGE.** Same shape of change as agency_abstraction; the 8 results keys move under `results`.
- **`aesthetic_authority_audit` CLI JSON output shape is a BREAKING CHANGE.** Pre-1.83 emitted the legacy block at the top level (`signal_path`, `family`, `status`, `aic_9_kicker_density`, etc.). Post-1.83 carries the envelope with the legacy block nested under `results`. Function-call consumers calling `aesthetic_authority_audit(text, nlp=...)` see no change.
- **`kicker_density` CLI JSON output shape is a BREAKING CHANGE.** Pre-1.83 emitted the legacy block at the top level. Post-1.83 carries the envelope with the legacy block nested under `results`. Function-call consumers calling `kicker_density(text, ...)` see no change — `variance_audit.compute_aic_9_kicker` and `aesthetic_authority_audit` continue to read `signal_path` / `family` / `value` from the function's return dict.
- **Existing CLI integration tests** for agency_abstraction, aesthetic_authority, and kicker_density updated to navigate into `data["results"]` for the legacy fields. `discourse_move_signature` had no CLI integration test pinning the legacy shape; only the new schema test was added.
- **Markdown rendering is unchanged.** All four scripts' `render_report` / inner block markdown remain bit-for-bit identical. Editorial-letter consumers see no change.

### Notes

- **Wave 2 of 7.** Subsequent waves: per-category audits (wave 3: `stance_modality_audit`, `punctuation_cadence_audit`, `phraseological_signature_audit`, `construction_signature_audit`, `function_word_grammar_audit`, `controls_audit`), voice surfaces (wave 4), smoothing surfaces (wave 5), validation surfaces (wave 6), craft restoration (wave 7). One PR per wave.
- **Function-call contracts preserved on the cross-script consumers.** Both `kicker_density` (called by `variance_audit` and `aesthetic_authority_audit`) and the per-detector helpers inside `aesthetic_authority_audit` retain their pre-1.83 function-return shapes. Only the CLI / `--json` path emits the new envelope. This means waves can land without touching variance_audit (which is wave 5).
- **No signal definitions, threshold values, computation, or markdown rendering change.** This is rendering-layer plumbing only.
- **Test suite status**: 2466 passed, 16 skipped, 0 failures (58 new tests for wave 2; 16 skips are pre-existing).

## [1.82.0] - 2026-05-17

**Output schema unification wave 1: `build_output()` helper + `aic_pattern_audit` proof migration.** First of seven waves landing a unified `schema_version` 1.0 JSON envelope across SETEC's ~30 audit/diagnostic scripts. Today every script rolls its own top-level keys, three incompatible `claim_license` serializations coexist (`pov_voice_profile`'s legacy 2-key dict, `phraseological_signature_audit`'s `{"rendered": ...}` markdown, the canonical `ClaimLicense.to_dict()` 11-key shape), baseline metadata lives in three different places (top-level `baseline_files_loaded` triplet vs. nested `baseline_comparison` vs. `baseline_stats`), and a handful of scripts lack `tool` / `version` / `available` entirely. This wave ships the helper that fixes that, plus one proof migration so downstream consumers can pin against the contract.

Internal SPEC at `internal/SPEC_output_schema_unification.md` (gitignored per repo convention) defines the target shape, per-script `results` taxonomies (per-pattern / per-category / per-signal), the 11-key `claim_license` contract, and the 7-wave migration order. This wave is the scaffolding; subsequent waves migrate scripts one bucket at a time, one PR per wave.

### Added

- **`output_schema.py`** — new module exposing `build_output(...)` and `build_baseline_metadata(...)`. Pins the schema_version 1.0 envelope: `schema_version`, `task_surface`, `tool`, `version`, `available`, `target`, `baseline`, `results`, `claim_license`, `claim_license_rendered`, `warnings`, `ai_status`. Validates task_surface against `VALID_TASK_SURFACES` (mirrors `claim_license.TASK_SURFACE_LABELS`), rejects mismatches between envelope `task_surface` and `claim_license.task_surface`, requires explicit `available=False` for null claim_license, and supports per-script extras via `target_extra` (merged into `target`) and `extra` (top-level, collision-checked against required keys).
- **`aic_pattern_audit.py`** migrated to the new envelope. Adds `TOOL_NAME` and `SCRIPT_VERSION` constants (the script lacked both), adds a `_claim_license()` helper (the script had no claim_license at all — per SPEC §11, scripts that lacked one gain a basic block as part of migration), splits `render_json` into `build_audit_payload` (returns the envelope dict) + `render_json` (json.dumps wrapper). Per-pattern data now lives under `results.patterns`; baseline metadata under the canonical `baseline` block.
- **20 new tests in `test_output_schema.py`** pinning the envelope keys, schema_version, validation rules (unknown task_surface, claim_license/envelope surface mismatch, available=True requires claim_license, extra-key collision), and `build_baseline_metadata` shape.
- **20 new tests in `test_aic_pattern_audit_schema.py`** pinning the envelope on the proof script: required-key presence, schema_version, no-legacy-keys invariant (`baseline_files_loaded` / `baseline_files_skipped` / `baseline_words` / `target_words` / `patterns` removed from the top level), 11-key `claim_license`, rendered-block header, `render_json` returns valid JSON, and the baseline-supplied path populates `baseline` and `results.patterns[*].baseline_density_per_1k`.

### Changed

- **`aic_pattern_audit` JSON output shape is a BREAKING CHANGE.** Pre-1.82 output had `task_surface`, `target`, `target_words`, `patterns` at the top level, with `baseline_files_loaded` / `baseline_files_skipped` / `baseline_words` added when `--baseline-dir` was supplied. Post-1.82 output has the full schema_version 1.0 envelope; per-pattern data lives under `results.patterns`, baseline metadata under `baseline`, and the script gains a structured `claim_license` block. Consumers that parsed pre-1.82 aic_pattern_audit JSON need to update. No legacy alias keys ship — per the SPEC, downstream consumers (APODICTIC) haven't shipped yet and the window for a clean break is open.
- **Markdown rendering is unchanged.** `render_report` continues to consume the audit dict and produce its existing markdown shape. Editorial-letter consumers see no change.

### Notes

- **Wave 1 of 7.** Subsequent waves migrate: per-pattern audits (`agency_abstraction_audit`, `discourse_move_signature`, `aesthetic_authority_audit`, `kicker_density`), per-category audits (`stance_modality_audit`, `punctuation_cadence_audit`, `phraseological_signature_audit`, `construction_signature_audit`, `function_word_grammar_audit`, `controls_audit`), voice surfaces (`voice_distance`, `voice_profile`, `voice_drift_tracker`, `pov_voice_profile`, `mimicry_cosplay_audit`, `idiolect_detector`), smoothing surfaces (`variance_audit`, `manuscript_audit`, `chapter_distinctiveness_audit`, `repetition_audit`, `manuscript_repetition_audit`, `bigram_diff`, `manuscript_bigram_diff`, `paragraph_audit`, `surprisal_audit`, `semantic_trajectory_audit`, `sliding_window_heatmap`), validation surfaces (`validation_harness`, `voice_validation_harness`, `general_imposters`, `manifest_validator`, `check_corpus`), and craft restoration (`restoration_packet`, `before_after_restoration`, `semantic_preservation_check`, `known_editor_profile`). One PR per wave.
- **No signal definitions, threshold values, computation, or markdown rendering change in this wave or any subsequent wave.** This is rendering-layer plumbing only.
- **Test suite status**: 2408 passed, 16 skipped, 0 failures (40 new tests added for this wave; 16 skips are pre-existing).
- **2.0.0 reservation.** The CHANGELOG preamble reserves major version bumps for breaking JSON contract changes. This wave starts the breaking change but does not finish it; 2.0.0 is reserved for when the full surface has migrated and the contract is stable.

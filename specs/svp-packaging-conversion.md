...
- **P1 — guards and zero-install skeleton, no production moves.** Land `scripts/setec/`, plugin path resolver, migration checker + one exemptions file, no-change claim-license guard, `pytest.ini` plus per-file pytest bootstrap conversion, and scratch-copy gates. Do not create `conftest.py`. Hermetic backend mode/cases are a separate follow-up amendment and PR.

...
The protected set is deliberately whole-module and conservative. Derive it independently from both the merge-base tree and candidate tree. Seed each with every production module whose AST defines or calls a name matching `_claim_license*`, plus every module that directly calls `ClaimLicense`. Resolve the names and arguments at those call sites only far enough to identify plugin-local imported modules that supply a referenced callable, constant, class attribute, or constructor argument; add each such supplier module in full. Repeat over newly added suppliers until the module set closes. Stdlib/builtin values and the unchanged `ClaimLicense` class are terminals. A base-only path is a deletion and a candidate-only path is an additive protected surface; both fail P1.

For every protected merge-base module, P1 compares the **whole-module** AST at the same path with `ast.dump(..., include_attributes=False)`; it retains docstrings, literals, operators, calls, keyword names/values, collection order, defaults, and control flow. This broad comparison catches caller-side argument changes such as a changed `model_id` even when `_claim_license` itself is untouched.

There is no claim of general semantic or interprocedural dataflow analysis. On a star import, dynamic `getattr`/`globals` lookup, ambiguous alias, or other unresolved provenance, the checker adds the entire identifiable plugin-local source module to the protected set; if it cannot identify a unique source module, it fails closed. It also fails on a missing merge base, a protected-set addition/deletion, or any whole-module AST delta. Existing live contract builders remain a second oracle where they exist, but fixture coverage is not misrepresented as universal.

P2 extends this gate only with its first concrete L0 relocation. That extension must scope each normalization to its exact old/new module pair, prove every structural rewrite has the expected occurrence cardinality, and test the real path-anchor equivalence against byte-identical scratch data. A global rename or dumped-text substitution is never authorized.

**No semantic claim-license change is authorized by this packaging spec.** Any change to a protected module's `licenses`, `does_not_license`, `comparison_set`, `additional_caveats` content, or `ai_status` semantics — that is, any delta inside a `_claim_license*`/`ClaimLicense(...)` call-site subtree, exactly as `tools/check_claim_license_guard.py` already identifies those subtrees to seed the closure — fails CI with no override, full stop. A change confined to a protected module's code outside those subtrees is eligible only for the structurally verified additive exemption defined in `specs/svp-hermetic-mode-amendment.md` §2 — never for an author-asserted, data-driven, or text-substitution override of any shape (see that amendment's record of the closed `moves[].old_symbol` channel). A genuine semantic content change, inside a protected subtree, must still be a later, separately reviewed PR under its own contract. This is both stronger and smaller than an owner-signature mechanism, a regenerable snapshot, or a new approval-artifact format. CI also fails if the merge base is unavailable. PR-body prose is irrelevant.
...
P1 deliberately does not add a synthetic backend mode. Its scratch-copy gate structurally resolves every manifest script path, executes representative top-level and nested launcher classes, and requires one real normalized dispatch to return the exact expected schema/tool/surface success envelope. The broader hermetic consumer matrix is deferred to `specs/svp-hermetic-mode-amendment.md` and its own reviewed PR (phase P1-hermetic); it is not an exemption or an implied part of P1.
...
- **P1 — guards and zero-install skeleton, no production moves.** Land `scripts/setec/`, plugin path resolver, migration checker + one exemptions file, no-change claim-license guard, `pytest.ini` plus per-file pytest bootstrap conversion, and scratch-copy gates. Do not create `conftest.py`. Hermetic backend mode/cases are phase P1-hermetic, authorized by `specs/svp-hermetic-mode-amendment.md` and built in its own separate follow-up PR.
...
- **P4 — relocate whole surfaces family by family.** Land package-aware drift discovery and launcher conformance with the first family. Each family is a separate, relocation-only PR: no boilerplate extraction, harness, output-builder edit, or claim-license edit. Nested capability paths receive per-file launchers, never directory aliases. The claim-license no-change guard remains mandatory. Producer C2 may proceed after P1 at the stay-put paths; P2 owns the later relocation and compatibility shims.
...
4. Scratch-copy structural reachability covers the runtime-derived manifest set; direct execution covers every nested script-path class; one normalized dispatch validates an exact successful consumer envelope. The deferred hermetic PR owns full consumer-case execution.
...
| Optional backends obscure zero-install failures | P1 requires one real successful dispatch; the separate hermetic-mode PR owns deterministic backend coverage and its case table |

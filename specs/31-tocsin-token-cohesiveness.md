...
```jsonc
{
  ...
  "effective_deletions": 74,            // floor(deletion_fraction * target_tokens) — tokens ACTUALLY dropped per perturbation; 0 => degenerate (see band guard)
  ...
  "band": {                             // CALIBRATED BAND — descriptive, not a verdict
    ...
    "flags": ["cohesiveness_high"],     // descriptive flags, never "ai"/"human"; "no_deletion_degenerate" when effective_deletions == 0
    ...
    "m1_band_reachability": "M1 stdlib proxy SATURATES near 1.0 on real prose at the default deletion_fraction; the default M1 band reaches only 'high_cohesiveness' — lower arms need a larger deletion_fraction or the M2 backend"
  },
  "assumptions": {
...
3. **Recursive-walk guard (R4 / `validate_results_bounds`).** `build_output(...)` recursively walks `results`. The payload is all finite numerics (cohesiveness/SD/diffs in `[0,1]`, integer counts) plus strings — it passes the unconditional NaN/inf finiteness check, and `token_cohesiveness`/`mean_semantic_diff`/`cohesiveness_sd` carry no `surprisal`/`perplexity`/`entropy`/`probability` token in their key names (verified against `output_schema._SURPRISAL_RE` / `_PROBABILITY_RE`), so they trip **no** range gate falsely. **The `[0,1]` bound is delivered by a CLAMP at the computing surface, not by a name-based R4 assertion:** because the R4 gate range-checks only the surprisal/probability key families, the injected `semantic_diff` (the M1/M2 seam — M2's `1 - cosine` can reach `[0,2]`, a buggy backend anything) is clamped per-perturbation into `[0,1]` (`d = min(1.0, max(0.0, raw))`) before reduction, mirroring how cosine is "clamped at the computing surface" per `output_schema`'s own comment. **NaN/inf is deliberately NOT clamped** — it must survive to the R4 finiteness check (which rejects it) rather than be hidden as a `0.0`. An acceptance test injects a `1.5` / `-0.5` diff and asserts `token_cohesiveness` stays in `[0,1]` (the failing test that backs the "bounded" word), and a second asserts NaN still raises `OutputValidityError`. A further test asserts **no verdict/label key string** appears anywhere in the recursively-walked envelope (see §6, AC-3).
4. **Degenerate-perturbation guard (fail toward NO reading).** When `floor(deletion_fraction * target_tokens) == 0` (any text with fewer than `1 / deletion_fraction` word tokens — 10 at the default 0.10) NO perturbation changes the text, so `mean_semantic_diff` is a vacuous `0.0` and `token_cohesiveness` is a vacuous `1.0`. The band must NOT read that ceiling as `high_cohesiveness` (the paper's "more-LLM-like" DIRECTION) on a non-event — that would be a fail-toward-the-inference-target artifact. Instead the band is forced to `indeterminate` with a `no_deletion_degenerate` flag, and `results.effective_deletions` records the count actually dropped. An acceptance test asserts a `<10`-token target at the default fraction does NOT emit `high_cohesiveness` (see §6, AC-3).
5. **Claim-license refusal.** The `does_not_license` block explicitly refuses any AI/human determination, any thresholded authorship verdict, and names the orthogonality + corpus-dependence caveats (mirrors `intrinsic_dimension_audit` / `fast_detect_curvature` / `originality_audit`).
...
**Functional**
- **AC-1.** `audit_tocsin(text, semantic_diff=<stub>)` returns the exact §3 `results` shape (incl. `effective_deletions`) with `token_cohesiveness ∈ [0,1]`, `cohesiveness_sd ≥ 0`, integer counts, and `deletion_unit == "word_token"` on the M1 path. Deterministic under a fixed seed (same input ⇒ byte-identical `results` across runs). The `[0,1]` bound holds on the **injected seam**: a test injecting a `1.5` / `-0.5` `semantic_diff` asserts `token_cohesiveness` stays in `[0,1]` (the clamp at the computing surface), the failing test that backs the "bounded" word.
...
- **AC-3 (no-verdict recursive walk + degenerate guard + real-path band).** A test recursively walks the FULL envelope (`results`, `band`, `claim_license`, every nested dict/list) and asserts that **no key and no string value** matches `{is_ai, is_human, ai_generated, human_written, label, prediction, classification, verdict, decision, p_ai, prob_ai}` (case-insensitive). The only categorical leaf permitted is `band.band ∈ {indeterminate, low_cohesiveness, high_cohesiveness}`. Two backstops: (a) a `<10`-token target at the default fraction (`effective_deletions == 0`) emits `indeterminate` + `no_deletion_degenerate`, **never** `high_cohesiveness` (fail toward NO reading, not toward the inference target); (b) a test over the REAL jaccard default path (no stub) pins the disclosed M1 saturation — the default band reaches only `high_cohesiveness` — so the AC-3 stub-driven band test cannot stand in for actual M1 behavior.
...
- **M1 proxy SATURATES near the ceiling (disclosed, not a 3-way classifier on the default path).** The M1 stdlib semantic difference is a **set-level** token Jaccard distance: deleting `floor(deletion_fraction * N)` positions removes a type only when its last occurrence is dropped, so the distance is bounded by `~deletion_fraction` (`≈ 0.10` at the default) and is *far* smaller on real prose (TTR < 1 means most deleted positions leave their type in the set). Measured: a ~330-token argument paragraph reads `token_cohesiveness ≈ 0.999`, repetitive text `1.0000`. Consequence: at the shipped thresholds (`high_above 0.85`, `low_below 0.65`) the **default M1 band reaches only `high_cohesiveness`** — the `indeterminate` and `low_cohesiveness` arms are mathematically unreachable on the default path. This is disclosed in the claim-license, `results.assumptions.m1_saturation`, and `band.m1_band_reachability`; the M1 per-input discrimination signal lives mostly in the **dispersion** (`cohesiveness_sd`), not the near-ceiling point value. The lower band arms and a wider-moving scalar require a **larger `deletion_fraction`** or the **M2 embedding backend** (whose `1 - cosine` moves on real prose). A test over the REAL jaccard path pins this saturation so a future metric/threshold change that makes the lower arms reachable will surface as a deliberate change to that test (anti-regression for the disclosed limitation). Unlike `intrinsic_dimension_audit` (whose M1 requires a real injected embedder), TOCSIN ships the degenerate proxy as the DEFAULT path, so the disclosure is load-bearing.

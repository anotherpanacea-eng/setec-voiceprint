...
- Emits the canonical `output_schema.build_output` envelope: `results.lrr / log_rank_mean / log_rank_sd / log_rank_acf1 / lrr_excluded_positions / n_positions / scorer_backend`, a `band` block that ships **no default cutoff** (band `uncalibrated`, `thresholds: None` — §5; no invented numeric default, matching `fast_detect_curvature` / `binoculars_audit`), upgrading to a descriptive band over the LRR value's own axis (`low_lrr` / `indeterminate` / `high_lrr`, `calibration_status: heuristic` + `thresholds_operator_supplied_not_framework_calibrated`) **only** when the operator supplies both `--threshold-low` and `--threshold-high`, and an `assumptions` block surfacing the sign/direction convention, the rank-0 inf convention, the **length-ceiling refusal** (a target exceeding the scorer context window is chunked and refused with a `text_too_long` message, not silently mis-ranked), the ESL/non-native caveat, the proxy-scorer-dependence/generator-strength-inversion caveat, and the paper-status caveat.

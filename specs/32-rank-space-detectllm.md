...
> - **[Issue 3 — MODERATE] LRR is a ratio of sequence sums; rank-0 convention pinned.** Per DetectLLM (arXiv:2306.05540), LRR is `sum_t(surprisal_nats_t) / sum_t(log(rank_t + 1))` — the (negated) summed log-likelihood over the summed log-rank, i.e. the ratio of the two sequence sums (equivalently the ratio of the two per-token means) — **NOT** the mean of the per-token ratios `surprisal_nats_t / log(rank_t + 1)`. `log_rank_t = log(rank_t + 1)` is finite at rank 0 (`log(1) = 0.0`), so a rank-0 (most-probable) token contributes its surprisal to the **numerator** and `0` to the **denominator** — it is NOT dropped from the statistic. The only refusal is when the whole-sequence **denominator** `sum_t(log(rank_t + 1))` is `0` (every scored token is rank 0): the ratio is undefined and `lrr` is `None` (a refusal, not a fabricated 0), with `log_rank_zero_positions` recording the rank-0 count. **No `inf`/`NaN` ever reaches the output envelope's `results`** (the aggregate scalars are always finite-or-`None`; the R4 finiteness gate is satisfied). Pinned by `test_rank_0_edge_case` and `test_lrr_is_ratio_of_sums_not_mean_of_ratios`. *(Round-9 P1 correction: the original spec/build computed the mean of per-token ratios and dropped rank-0 tokens; corrected to the ratio-of-sums DetectLLM definition.)*
...
2. `lrr` — `sum_t(surprisal_nats_t) / sum_t(log(rank_t + 1))`, the ratio of the two sequence sums (NOT the mean of per-token ratios). The DetectLLM-LRR statistic. `None` only when the sequence denominator is `0` (every token rank 0).
...
Log-rank at position `t` = the 0-indexed rank of `token_ids[t+1]` in the position-`t` vocab log-prob vector sorted **descending**. `log_rank_t = log(rank_t + 1)` (add-1 convention, DetectLLM), and `surprisal_nats_t = surprisal_bits_t / log2(e)`. LRR is then the **ratio of the two sequence sums**, `LRR = sum_t(surprisal_nats_t) / sum_t(log_rank_t)` — not a per-token ratio averaged over positions. A rank-0 token (`log_rank_t = 0`) still contributes `surprisal_nats_t` to the numerator.
...
- `rank_series_from_distributions(log_probs_nats, token_ids, surprisal_bits) -> {"log_rank_series", "surprisal_nats_series"}`. Pure-Python over the `score_text_with_distributions` tuple (or an injected stub of the same shape). Returns the two SUMMABLE LRR components (denominator terms `log(rank+1)` and numerator terms `surprisal_nats`); rank-0 keeps its surprisal in the numerator series and `0` in the log-rank series. Descending sort; add-1 convention. Validates input length consistency (raises `ValueError` on a tokenization/wiring mismatch) and a token-id out-of-range (raises `IndexError`).
- `aggregate_rank_signals(log_rank_series, surprisal_nats_series, surprisal_bits) -> {log_rank_mean, log_rank_sd, log_rank_acf1, lrr, log_rank_zero_positions, n_positions}`. All scalars finite-or-`None`; `lrr = sum(surprisal_nats_series) / sum(log_rank_series)` (ratio of sequence sums), `None` only when the sequence denominator `sum(log_rank_series)` is `0`; SD is `None` < 2 points, ACF `None` < 3 points. ACF is the same biased lag-1 estimator as `surprisal_audit._acf_at_lag` (Pearson on the lag-paired series); the structural minimum is 3 (not surprisal_audit's tuned 30-token floor — documented in the helper).
...
- Emits the canonical `output_schema.build_output` envelope: `results.lrr / log_rank_mean / log_rank_sd / log_rank_acf1 / log_rank_zero_positions / n_positions / scorer_backend`, a `band` block that ships **no default cutoff** (band `uncalibrated`, `thresholds: None` — §5; no invented numeric default, matching `fast_detect_curvature` / `binoculars_audit`), upgrading to a descriptive band over the LRR value's own axis (`low_lrr` / `indeterminate` / `high_lrr`, `calibration_status: heuristic` + `thresholds_operator_supplied_not_framework_calibrated`) **only** when the operator supplies both `--threshold-low` and `--threshold-high`, and an `assumptions` block surfacing the sign/direction convention, the LRR ratio-of-sums + rank-0 convention, the **length-ceiling refusal** (a target exceeding the scorer context window is chunked and refused with a `text_too_long` message, not silently mis-ranked), the ESL/non-native caveat, the proxy-scorer-dependence/generator-strength-inversion caveat, and the paper-status caveat.

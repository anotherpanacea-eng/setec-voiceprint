...
| [`21-attribution-refusal-lab.md`](21-attribution-refusal-lab.md) | **Research brief / anti-goal note — NOT a dispatchable spec** (no Contract/Test-contract by design). Refusal-curve lab for the no-verdict-about-the-person cluster (open-set attribution / demographic profiling / identity linkage): reports strength-of-evidence only (never an identity), with a testable E3 non-leak/redaction rule. Becomes builder-dispatchable only if promoted to a full spec. No task surface, script, or `capabilities.d/` entry ships. |

...
| [`21-attribution-refusal-lab.md`](21-attribution-refusal-lab.md) | **Spec (research; gated).** Refusal-curve lab for the no-verdict-about-the-person cluster (open-set attribution / demographic profiling / identity linkage). Reports strength-of-evidence only (never an identity); the deliverable is the demonstration of the signal's limits + the untooled-LLM contrast. No task surface, script, or `capabilities.d/` entry ships. |

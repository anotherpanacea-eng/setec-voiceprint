...
| [`73-ci-test-topology.md`](73-ci-test-topology.md) | **Specification review GO; instrumented monolithic baseline built; CI measurement pending.** Deterministic, collection-conserving Linux CI lanes for unit shards, serial subprocess/CLI tests, and narrow integration contracts while preserving every focused native-platform job and the `pytest` merge gate. |

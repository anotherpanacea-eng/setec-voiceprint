...
- **Upstream / prior art:** see `specs/00-stylometry-kit-research-brief.md` §… and the voice-matching brief — "Catch Me If You Can? Not Yet" (EMNLP 2025), StyleAdaptedLM ([2507.18294](https://arxiv.org/abs/2507.18294)), DOC ([2212.10077](https://arxiv.org/abs/2212.10077)), LUAR (Apache-2.0), Wegmann content-controlled embeddings.
...
(`references/setec-normalized-entrypoint-spec.md`, #139) + the APODICTIC handoff
...
- **Entry point:** the normalized `setec run <surface> --json` dispatcher (#139)
  over `build_output()` envelopes; pin only `handoff: stable` surfaces
  (`variance_audit`, `voice_distance`, `idiolect_detector`, `aic_pattern_audit`,
  `restoration_packet`) + the planned **voice-fingerprint embedding surface
  (`specs/02`)**, which is the matcher's lead evaluator and should ship first.

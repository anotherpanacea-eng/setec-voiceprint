...
- **Multi-register author-corpus producer (R1a, reviewed build in progress).**
  `author_corpus_export` packages the user's own sent Messages/Gmail and explicitly
  attested local author documents behind explicit `text.*` / `email.*` / long-form
  registers, HMAC-pseudonymous stable ordered groups, a standard no-prose
  normalized-entrypoint receipt, and an enforced degraded/train-only posture when
...
### Passage-level hygiene for training corpora (2026-07-23)

The safeguards above are *document*-level. `acquisition_core.content_hash_already_present()` drops byte-exact duplicates within one output directory; `near_dup_dedup.py` (MinHash-LSH candidate retrieval, exact 5-word-shingle Jaccard confirmation at 0.8, deterministic representative) drops near-duplicate *documents* across manifests. LSH candidate false negatives can still retain duplicates, so document mode is optional hygiene rather than a uniqueness certificate. Neither lens sees a passage repeated inside or across otherwise-distinct documents.

A continued-pretraining run on a private single-author corpus surfaced the gap empirically: a generated output reproduced a **contiguous 41-token training-corpus span**. Byte-exact document dedup had already run over that corpus, so the reproduced span is *not* attributable to a whole-document duplicate — it is a repeated passage the document-level lens cannot see. Repeated passages are also the fastest-memorized material under multi-epoch continued pretraining, which is the regime that run operated in. The finding licenses exactly one claim — a contiguous span was reproduced and its source is not a whole-document duplicate — and specifically does **not** show that memorization worsened: on paired probes the aggregate direction is unresolved (exact McNemar `p = 0.453`), and the unadapted base arm produced violations of its own, so no absolute memorization rate is claimable from it. The full calibration record is held in the operator's private fleet hub.

Three follow-ups, none requiring a new algorithm:

- **Passage-level dedup mode.** Chunk documents into passages, emit as a manifest, reuse the existing `near_dup_dedup` LSH path. The shingle helper is already exported; what is missing is the chunking driver plus a passage→source-document provenance record, so a dropped passage remains traceable to its document. Constraint: duplicate-dependent surfaces must **not** consume a passage-deduped corpus — their by-design pools depend on retained duplicates. A grep-derived sweep during spec 36 found **five** such surfaces, not the three first listed here: `corpus_novelty_audit`, `homogeneity_audit` (pool mode), `distinct_diversity_audit`, `skeleton_overlap_audit`, and `cross_doc_novelty_profile`. `originality_audit` declares the same `set_level_diversity` task surface but is **exempt** — duplicate pool members are idempotent for its longest-match coverage — so the guard must key on purpose, not on the surface tag. Three of the five carry clean-room copies of the pool loader rather than importing the shared one, which rules out any loader-signature-based guard; see `specs/36-passage-level-corpus-hygiene.md` for the authoritative table and mechanism.
- **Register sweep CLI (H2 re-scoped; spec 73) — LANDED 2026-07-26 via PR #361 (merge `48f1a13`).** `register_classifier.py` exposes `classify_register()` and `register_match()`; the corpus runner shipped as `register_sweep.py` (capability `register_composition_sweep`). H2 is only an aggregate hygiene screen asking whether an explicitly scoped corpus is obviously register-mixed enough to prompt a hand-check. Register is a confounded heuristic proxy: the sweep does not explain author-reference multimodality or semantic modes, does not produce a calibrated/reportable distribution, and licenses no selection, disposition, activation, registration, or training action. Free-text `source`, per-document-identity `source_id`, and owner-approved optional categorical `source_family` are all excluded from the projection and every H2 identity. Any source-family analysis is a separate reviewed follow-on, never H2 grouping.
- **Reconstructibility-targeted memorization probes.** `corpus_novelty_audit.py` already computes leave-one-out DJ-Search reconstructibility per document, from the corpus alone with no generation required. Ranking by it yields a memorization *prior* that can target a probe set at the high-risk tail rather than sampling prefixes uniformly — materially more power per probe, and precommittable, because it is computed before any generation exists.

Unimodality itself needs no build *at reference scale*: `homogeneity_audit.py --manifest` already reports `effective_modes` (participation ratio over the Gram eigenvalues) beside the pairwise-cosine distribution. **Scale-bounded — see "Corpus-scale and registry-contract gaps" below.** The 2026-07-24 hygiene pass ran it successfully on a ~1k-row reference but had to refuse a ~10k-row proposal.

**Status (spec 36 M1 built, Codex fixes 2026-07-24; document safety repair #407 prepared 2026-08-23).** The passage-level dedup mode shipped as `near_dup_dedup.py --passages`, but **not** as the single-pass reuse of the LSH path sketched in the first bullet above — spec review's arithmetic retracted that: a 41-token span shared between two ~120-word passages scores Jaccard ≈ 0.19, so the motivating case is invisible to *any* passage-unit similarity threshold, and greedy coalescing of short paragraphs makes it worse. The shipped design is two-stage: **Stage A** near-duplicate raw-paragraph units (uncoalesced; token-empty sub-floor units use raw-text equality; a complete frequency-ordered Jaccard prefix index with length/positional upper bounds prevents LSH false negatives without a common-boilerplate exact-comparison explosion; exact rational arithmetic preserves equality boundaries and exact Jaccard alone decides) and **Stage B** an exact stdlib inverted-8-shingle span scan whose continuation follows repeated occurrence subsets, preserving the arithmetic guarantee even when one shingle has extra occurrences. Passage runs emit flushed progress and transactionally checkpoint Stage-A passage/pair shards plus Stage-B per-document token/key/region/span shards under exact bindings; corpus-derived shards live in a tool-owned private 0700 directory/0600 SQLite database without changing the user checkpoint parent's mode. Exports refuse skipped rows, portable filename collisions, and nested output layouts in either direction, stage sidecars, and publish the manifest last. The mechanical guard shipped as `pool_guard.py` — a per-surface file-level marker scan (a shared-loader kwarg cannot reach the clean-room copies) plus `plugins/setec-voiceprint/scripts/tests/test_pool_guard_coverage.py`, which pins the complete nine-module classification map with a mandatory rationale on every entry, firing and exempt alike. The document-mode #407 repair leaves both passage stages unchanged: LSH is candidate generation only, every destructive union edge is exact-confirmed, and reports bind candidate signature/package identity plus the ephemeral-rebuild-only index policy. The **register sweep CLI was deferred, not dropped** — and has since landed. Spec 37 repaired the former taxonomy/scorer/tie defects; re-scoped Spec 73 carried the source-free aggregate hygiene-screen contract, cleared independent review, and its implementation merged 2026-07-26 via PR #361 (merge `48f1a13`) after the Spec 76 land and the immutable H1 receipt/checker gate both cleared. Neither the spec nor the landed tool authorizes a private corpus run; running one remains an operator decision. The memorization-probe bullet is unchanged and still a training-side project.

**Status (Spec 75 H6 M1 implementation, 2026-07-24).** The targeted-probe
builder now freezes a distinct, pre-generation qualification/sealed probe
package from the owner-attested training snapshot's high reconstructibility
tail. This does not replace or reinterpret the completed SHA-ordered `n=500`
battery: that battery remains the identical-set historical characterization,
while the new builder prepares a separately hash-bound input for a future
matched-arm re-baseline. Building the package is not a memorization result and
authorizes no model run, training continuation, corpus activation, checkpoint
selection, or deployment.

**Status (Spec 37 H1 merged in PR #348; Spec 76 H1 built and merged, 2026-07-24).** The classifier-only prerequisite now uses the versioned
`register_families/v2` vocabulary: all canonical manifest registers and ten
deprecated classifier inputs resolve through one family-first mapping, all
eight emittable families have distinct scorers, exact top ties refuse, and
...
updated post-clearance to record the landed closeout — the normative contract,
acceptance tests, and build sequence were byte-identical to the reviewed bytes
(verifiable by diffing the two blobs). It now hashes to `e9f49c89…`: the status
header was updated to landed and four post-implementation amendments were
folded in (classifier digest-check order, crosstab row-marginal bound, explicit
23-key report count, owner-trusted checkpoint integrity model). That delta is
diffable against the `80dbf2f4…` blob; the same dual-hash provenance rule
applies. The
immutable H1 receipt/checker closeout is implemented and has strictly
validated merged PR #352, its preserved review heads/artifact bytes, and
landed-main push run `30131248170` attempt 1, and that closeout landed in merge
commit `c6d7cbc`. **H2 implementation LANDED 2026-07-26 via PR #361** (merge
`48f1a13`) as the `register_composition_sweep` capability; the receipt's raw
SHA-256 is pinned as the implementation constant `H1_RECEIPT_SHA256`. Neither
the spec nor the landed tool authorizes a private corpus run. APODICTIC and setec-voicewright
fixture/pin migrations remain required
before consumer-ready promotion.

### Corpus-scale and registry-contract gaps (2026-07-24)

Two gaps surfaced by the first read-only corpus-hygiene pass over staged proposals. Neither is a defect in shipped behaviour — the tools refused honestly rather than degrading — but both bound what the hygiene lane can currently answer, and neither was tracked anywhere before this entry.

- **`homogeneity_audit` does not scale past reference-sized sets.** The pool path is all-pairs: it materializes every pairwise cosine and a dense Gram matrix, with no checkpointing and no streaming. It completed on a ~1,000-row registered reference (~525k pairs) but was **refused on a ~10,000-row staged proposal**, where the same implementation would have materialized ~19M pairs and a ~38M-cell dense matrix. The refusal was the correct behaviour and **no sample was silently substituted** — but it means the mixture question is currently unanswerable at proposal scale, and every future proposal-scale corpus hits the same wall. Options, cheapest first: (a) a checkpointed/blocked Gram accumulation so the matrix never lives in memory at once; (b) a documented, *explicitly-requested* sampling mode that reports its own sampling error — never an implicit fallback, since a silent sample would turn a refusal into a fabricated distribution; (c) a streaming estimator for `effective_modes` alone, since the participation ratio needs the eigenvalue spectrum rather than the stored matrix. Any of these needs a spec before build; the no-verdict posture and the "distribution + representatives, never a lone scalar" framing carry over unchanged.

- **The source-mixture question now has an owner-approved registry axis; the public field contract is landed and the separate private population is complete.** Asking "does a given source form its own mode in the author reference?" requires a stable categorical grouping key. Free-text `source` is unsuitable, and existing `source_id` values are per-document identity slugs that must remain byte-unchanged. The optional `source_family` field therefore accepts only `facebook`, `metafilter`, `wordpress`, or `unclassified`. The public validator checks values that are explicitly supplied; it does not infer them from paths, acquisition details, `source`, `source_id`, or register, and this increment adds no public path mapping or domain registry. Source-level mixture analysis remains separate work even though the private baseline is now populated and validates. This source-family thread is separate from H2: the register sweep remains a weaker register-mixing hygiene screen because register is confounded with topic, project, date, source, and length, and the measured register association was small and composition-sensitive.

Both items are corpus-lane preconditions rather than blockers on the passage-hygiene work above, which stands on its own.
...
Cathedral upgrade #3 (validation harness) and the threshold-calibration prerequisite need labeled human-vs-AI corpora. The calibration toolchain shipped in 1.10.0 with a license-aware fetcher for Pangram Labs' EditLens (CC BY-NC-SA 4.0, gated; local-only). RAID and MAGE fetchers shipped in 1.42.x; SETEC records each pinned repository wrapper-license declaration while keeping corpus content local-only:

- **`scripts/calibration/fetch_raid.py` — shipped.** RAID benchmark (Dugan et al., NAACL 2024; current HuggingFace repository wrapper metadata at `liamdugan/raid` declares MIT). The hosted `train.csv`, `test.csv`, and `extra.csv` co-locate attack and non-attack rows, so `--no-adversarial` cannot reduce the fetch; row-level exclusion belongs to `raid_to_manifest.py --no-adversarial`. Corpus content stays local-only.

- **`scripts/calibration/fetch_mage.py` — shipped.** MAGE benchmark (Yichen Li et al., ACL 2024; current HuggingFace repository wrapper metadata at `yaful/MAGE` declares Apache-2.0). MAGE aggregates source datasets with their own terms, so the wrapper declaration is recorded without asserting a blanket per-row license; corpus content stays local-only. Companion to RAID; the four-benchmark empirical frame `references/implementation-survey.md` records is RAID + MAGE + MAGE-extension + Ghostbuster.

...
The four are independently shippable. RAID and MAGE shipped in 1.42.x (the section above is mildly stale); the PROVENANCE template shipped in 1.29.0. The fiction impostor pool is the next concrete acquisition target, gated on whether the maintainer needs the GI-on-fiction pathway before or after the AMD calibration runs complete.

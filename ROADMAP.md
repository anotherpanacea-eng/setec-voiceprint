...
- **Register sweep CLI.** `register_classifier.py` exposes `classify_register()` and `register_match()` but has no `__main__`, and only `voice_distance.py` imports it, so a corpus cannot currently be swept for register composition without a caller writing its own driver. Needed because a training reference spanning registers is a *mixture, not a mode*, which depresses achievable voice similarity for reasons no additional training can fix. Preserve the module's own framing: heuristic, not labeled-corpus-validated — the sweep screens for obvious mixture and prompts a hand-check; it does not produce a reportable register distribution.
...
- **The source-mixture question is blocked on a manifest-contract decision, not on compute.** Asking "does a given source form its own mode in the author reference?" requires a stable categorical grouping key. The manifest has no such field in practice: `source` is populated on only a minority of rows and is *overwhelmingly singleton-valued* (essentially a free-text provenance note), and no populated stable `source_id` exists at all. The 2026-07-24 pass therefore **froze the question as unanswerable from registered metadata and inferred no labels** — the right call, since inferring source groupings from free-text provenance would manufacture the very structure the test is supposed to detect. Unblocking it is a **registry-contract decision for the owner**, not a build task: decide whether to define, populate, and validate a stable categorical `source_id` (with an allowed-value enumeration in `manifest_validator`, like `register` has), and whether back-population of existing rows is in scope. Until that lands, source-level mixture claims are out of reach, and the `register`-based decomposition is a *different, weaker* question — it is a contract-defined categorical axis, but register is confounded with topic, project, date, source, and length, and the measured register association was small and composition-sensitive.

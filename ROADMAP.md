...
**`tools/check_docs_freshness.py` crashes on a non-UTF-8 (Windows) console.** The gate logic passes, but it then `print`s a success line containing `✔` (U+2714), which raises `UnicodeEncodeError` under a cp1252 default console (`PYTHONUTF8=1` makes it print cleanly). CI is on Linux so this never fires there; it only bites a maintainer running the gate locally on Windows. *Surfaced 2026-06-19 while propagating the AGENTS.md workflow refinement.* Same class of bug as the apodictic gate-parity fix (apodictic #115/#118): pin `PYTHONUTF8`/`PYTHONIOENCODING` for the tool, or use an ASCII success marker. Low urgency (Windows-local only); fix when convenient. Audit sibling tooling for the same `✔`/unicode-to-stdout pattern at the same time.

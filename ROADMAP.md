...
5. **Report template: "what this result licenses / does not license."** Every harness output carries an explicit licensing block: inputs, comparison set, length range, register match, language match, confidence interval, FPR target, and the specific claim the result does and does not entitle. Status: **helper shipped** in `scripts/claim_license.py` (1.29.0). The `ClaimLicense` dataclass + `render_block()` produce a structured markdown licensing block; `from_legacy()` adapts existing dict-shaped `CLAIM_LICENSE` blocks the older harnesses carry. First wired through `sliding_window_heatmap.py`; incremental adoption across `validation_harness.py`, `voice_validation_harness.py`, and `general_imposters.py` is follow-up work.

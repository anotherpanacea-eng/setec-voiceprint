...
- **The source-mixture question now has an owner-approved registry axis, but that axis is not yet built/populated.** Asking "does a given source form its own mode in the author reference?" requires a stable categorical grouping key. Free-text `source` is unsuitable, and the 70 existing `source_id` values are near-singleton per-document identity slugs that must remain byte-unchanged. The 2026-07-24 owner ruling therefore adds a distinct optional `source_family` field with a frozen vocabulary, hand-authored path-prefix mapping, and domain-equality drift gate. Until that small registry build lands and the baseline is populated, source-level mixture claims remain out of reach; no labels are inferred from free-text provenance. This source-family thread is separate from H2: the register sweep remains a weaker register-mixing hygiene screen because register is confounded with topic, project, date, source, and length, and the measured register association was small and composition-sensitive.

...
The safeguards above are *document*-level. `acquisition_core.content_hash_already_present()` drops byte-exact duplicates within one output directory; `near_dup_dedup.py` (MinHash-LSH, 5-word shingles, Jaccard 0.8, deterministic representative) drops near-duplicate *documents* across manifests. Neither sees a passage repeated inside or across otherwise-distinct documents.

...
- **Register sweep CLI (H2 re-scoped; spec 73).** `register_classifier.py` exposes `classify_register()` and `register_match()` but has no corpus runner. H2 is only an aggregate hygiene screen asking whether an explicitly scoped corpus is obviously register-mixed enough to prompt a hand-check. Register is a confounded heuristic proxy: the sweep does not explain author-reference multimodality or semantic modes, does not produce a calibrated/reportable distribution, and licenses no selection, disposition, activation, registration, or training action. Free-text `source`, per-document-identity `source_id`, and owner-approved optional categorical `source_family` are all excluded from the projection and every H2 identity. Any source-family analysis is a separate reviewed follow-on, never H2 grouping.
...
repaired contract is independently clear at SHA-256 `c2202380…`; the file now
hashes to `80dbf2f4…` because its status header and dependency-state prose were
updated post-clearance to record the landed closeout — the normative contract,
acceptance tests, and build sequence are byte-identical to the reviewed bytes
(verifiable by diffing the two blobs). The
...
commit `c6d7cbc`. **H2 implementation is therefore unblocked and
unclaimed**; pinning the receipt's raw SHA-256 is its first implementation step. The spec and
